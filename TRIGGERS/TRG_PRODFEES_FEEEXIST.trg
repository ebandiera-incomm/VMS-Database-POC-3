CREATE OR REPLACE TRIGGER VMSCMS.trg_prodfees_feeexist
   BEFORE INSERT
   ON VMSCMS.CMS_PROD_FEES    FOR EACH ROW
DISABLE
DECLARE
   v_message            NUMBER                                := 0;
...
BEGIN                                                     --<< main begin >>--
   ...
   ***********************************************/
   ...
   THEN
        FOR x IN cur_prod_fees
         LOOP
     
            IF cur_prod_fees%ROWCOUNT > 0
            THEN
            ...
            EXIT WHEN cur_prod_fees%NOTFOUND;
