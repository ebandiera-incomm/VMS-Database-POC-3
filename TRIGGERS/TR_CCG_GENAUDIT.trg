CREATE OR REPLACE TRIGGER VMSCMS.TR_CCG_GENAUDIT
AFTER  INSERT OR UPDATE OR DELETE ON VMSCMS.CMS_CUST_CATG FOR EACH ROW
DECLARE
 ------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  v_table_nm := 'CMS_CUST_CATG' ;
  
  IF INSERTING THEN
    v_inst_code := :new.CCC_INST_CODE;
  ELSE
    v_inst_code := :old.CCC_INST_CODE;
  END IF;
...
P_DTL_INFO(1).COL_NAME := 'CCC_CATG_CODE'; -- NOT USED
P_DTL_INFO(2).COL_NAME := 'CCC_CATG_SNAME';
P_DTL_INFO(3).COL_NAME := 'CCC_CATG_DESC';
P_DTL_INFO(4).COL_NAME := 'CCC_INS_USER';
P_DTL_INFO(5).COL_NAME := 'CCC_INS_DATE';
P_DTL_INFO(6).COL_NAME := 'CCC_LUPD_USER';
P_DTL_INFO(7).COL_NAME := 'CCC_LUPD_DATE';


P_DTL_INFO(1).OLD_VAL := :OLD.CCC_CATG_CODE;
P_DTL_INFO(2).OLD_VAL := :OLD.CCC_CATG_SNAME;
P_DTL_INFO(3).OLD_VAL := :OLD.CCC_CATG_DESC;
P_DTL_INFO(4).OLD_VAL := :OLD.CCC_INS_USER;
P_DTL_INFO(5).OLD_VAL := :OLD.CCC_INS_DATE;
P_DTL_INFO(6).OLD_VAL := :OLD.CCC_LUPD_USER;
P_DTL_INFO(7).OLD_VAL := :OLD.CCC_LUPD_DATE;


P_DTL_INFO(1).NEW_VAL := :NEW.CCC_CATG_CODE;
P_DTL_INFO(2).NEW_VAL := :NEW.CCC_CATG_SNAME;
P_DTL_INFO(3).NEW_VAL := :NEW.CCC_CATG_DESC;
P_DTL_INFO(4).NEW_VAL := :NEW.CCC_INS_USER;
P_DTL_INFO(5).NEW_VAL := :NEW.CCC_INS_DATE;
P_DTL_INFO(6).NEW_VAL := :NEW.CCC_LUPD_USER;
P_DTL_INFO(7).NEW_VAL := :NEW.CCC_LUPD_DATE;
