CREATE OR REPLACE TRIGGER VMSCMS.TR_EMVPARAM_GENAUDIT
AFTER  INSERT OR UPDATE OR DELETE ON VMSCMS.CMS_EMV_PARAM FOR EACH ROW
DECLARE
 ------------------------------------------------------------------------------------------------
...
BEGIN
        -- SN Initialize all table variables before itself
				...
				v_table_nm := 'CMS_EMV_PARAM';
        
      IF INSERTING THEN
        v_inst_code := :new.CEP_INST_CODE;
      ELSE
        v_inst_code := :old.CEP_INST_CODE;
      END IF;
...
P_DTL_INFO(1).COL_NAME := 'CEP_PARAM_KEY';
P_DTL_INFO(2).COL_NAME := 'CEP_PARAM_DESC';
P_DTL_INFO(3).COL_NAME := 'CEP_PARAM_VALUE';
P_DTL_INFO(4).COL_NAME := 'CEP_PARAM_TYPE';
P_DTL_INFO(5).COL_NAME := 'CEP_INS_USER';
P_DTL_INFO(6).COL_NAME := 'CEP_INS_DATE';
P_DTL_INFO(7).COL_NAME := 'CEP_LUPD_USER';
P_DTL_INFO(8).COL_NAME := 'CEP_LUPD_DATE';


P_DTL_INFO(1).OLD_VAL := :OLD.CEP_PARAM_KEY;
P_DTL_INFO(2).OLD_VAL := :OLD.CEP_PARAM_DESC;
P_DTL_INFO(3).OLD_VAL := :OLD.CEP_PARAM_VALUE;
P_DTL_INFO(4).OLD_VAL := :OLD.CEP_PARAM_TYPE;
P_DTL_INFO(5).OLD_VAL := :OLD.CEP_INS_USER;
P_DTL_INFO(6).OLD_VAL := :OLD.CEP_INS_DATE;
P_DTL_INFO(7).OLD_VAL := :OLD.CEP_LUPD_USER;
P_DTL_INFO(8).OLD_VAL := :OLD.CEP_LUPD_DATE;


P_DTL_INFO(1).NEW_VAL := :NEW.CEP_PARAM_KEY;
P_DTL_INFO(2).NEW_VAL := :NEW.CEP_PARAM_DESC;
P_DTL_INFO(3).NEW_VAL := :NEW.CEP_PARAM_VALUE;
P_DTL_INFO(4).NEW_VAL := :NEW.CEP_PARAM_TYPE;
P_DTL_INFO(5).NEW_VAL := :NEW.CEP_INS_USER;
P_DTL_INFO(6).NEW_VAL := :NEW.CEP_INS_DATE;
P_DTL_INFO(7).NEW_VAL := :NEW.CEP_LUPD_USER;
P_DTL_INFO(8).NEW_VAL := :NEW.CEP_LUPD_DATE;
