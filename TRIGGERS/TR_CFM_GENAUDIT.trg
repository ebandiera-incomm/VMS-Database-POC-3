CREATE OR REPLACE TRIGGER VMSCMS.TR_CFM_GENAUDIT
...
BEGIN
		...
		v_table_nm := 'CMS_FEE_MAST'  ;
    
    IF INSERTING THEN
      v_inst_code := :new.CFM_INST_CODE;
    ELSE
      v_inst_code := :old.CFM_INST_CODE;
    END IF;
...
P_DTL_INFO(1).COL_NAME := 'CFM_FEETYPE_CODE';
P_DTL_INFO(2).COL_NAME := 'CFM_FEE_CODE';      -- NOT USED
P_DTL_INFO(3).COL_NAME := 'CFM_FEE_AMT';
P_DTL_INFO(4).COL_NAME := 'CFM_FEE_DESC';
P_DTL_INFO(5).COL_NAME := 'CFM_INS_USER';
P_DTL_INFO(6).COL_NAME := 'CFM_INS_DATE';
P_DTL_INFO(7).COL_NAME := 'CFM_LUPD_USER';
P_DTL_INFO(8).COL_NAME := 'CFM_LUPD_DATE';
P_DTL_INFO(1).OLD_VAL := :OLD.CFM_FEETYPE_CODE;
P_DTL_INFO(2).OLD_VAL := :OLD.CFM_FEE_CODE;
P_DTL_INFO(3).OLD_VAL := :OLD.CFM_FEE_AMT;
P_DTL_INFO(4).OLD_VAL := :OLD.CFM_FEE_DESC;
P_DTL_INFO(5).OLD_VAL := :OLD.CFM_INS_USER;
P_DTL_INFO(6).OLD_VAL := :OLD.CFM_INS_DATE;
P_DTL_INFO(7).OLD_VAL := :OLD.CFM_LUPD_USER;
P_DTL_INFO(8).OLD_VAL := :OLD.CFM_LUPD_DATE;
P_DTL_INFO(1).NEW_VAL := :NEW.CFM_FEETYPE_CODE;
P_DTL_INFO(2).NEW_VAL := :NEW.CFM_FEE_CODE;
P_DTL_INFO(3).NEW_VAL := :NEW.CFM_FEE_AMT;
P_DTL_INFO(4).NEW_VAL := :NEW.CFM_FEE_DESC;
P_DTL_INFO(5).NEW_VAL := :NEW.CFM_INS_USER;
P_DTL_INFO(6).NEW_VAL := :NEW.CFM_INS_DATE;
P_DTL_INFO(7).NEW_VAL := :NEW.CFM_LUPD_USER;
P_DTL_INFO(8).NEW_VAL := :NEW.CFM_LUPD_DATE;
