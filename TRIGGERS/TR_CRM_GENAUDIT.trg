CREATE OR REPLACE TRIGGER VMSCMS.TR_CRM_GENAUDIT
AFTER  INSERT OR UPDATE OR DELETE ON VMSCMS.CMS_REGION_MAST FOR EACH ROW
DECLARE
 ------------------------------------------------------------------------------------------------
...
BEGIN
		...
		v_table_nm := 'CMS_REGION_MAST' ;
    
    IF INSERTING THEN
      v_inst_code := :new.CRM_INST_CODE;
    ELSE
      v_inst_code := :old.CRM_INST_CODE;
    END IF;
...
P_DTL_INFO(1).COL_NAME := 'CRM_REGION_ID'; -- NOT USED
P_DTL_INFO(2).COL_NAME := 'CRM_REGION_NAME';
P_DTL_INFO(3).COL_NAME := 'CRM_INS_USER';
P_DTL_INFO(4).COL_NAME := 'CRM_INS_DATE';
P_DTL_INFO(5).COL_NAME := 'CRM_LUPD_USER';
P_DTL_INFO(6).COL_NAME := 'CRM_LUPD_DATE';
P_DTL_INFO(1).OLD_VAL := :OLD.CRM_REGION_ID;
P_DTL_INFO(2).OLD_VAL := :OLD.CRM_REGION_NAME;
P_DTL_INFO(3).OLD_VAL := :OLD.CRM_INS_USER;
P_DTL_INFO(4).OLD_VAL := :OLD.CRM_INS_DATE;
P_DTL_INFO(5).OLD_VAL := :OLD.CRM_LUPD_USER;
P_DTL_INFO(6).OLD_VAL := :OLD.CRM_LUPD_DATE;
P_DTL_INFO(1).NEW_VAL := :NEW.CRM_REGION_ID;
P_DTL_INFO(2).NEW_VAL := :NEW.CRM_REGION_NAME;
P_DTL_INFO(3).NEW_VAL := :NEW.CRM_INS_USER;
P_DTL_INFO(4).NEW_VAL := :NEW.CRM_INS_DATE;
P_DTL_INFO(5).NEW_VAL := :NEW.CRM_LUPD_USER;
P_DTL_INFO(6).NEW_VAL := :NEW.CRM_LUPD_DATE;
