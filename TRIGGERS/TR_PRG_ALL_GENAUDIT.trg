CREATE OR REPLACE TRIGGER VMSCMS.TR_PRG_ALL_GENAUDIT
AFTER  INSERT OR UPDATE OR DELETE ON VMSCMS.CMS_GROUP_PROG FOR EACH ROW
DECLARE
 ------------------------------------------------------------------------------------------------
...
BEGIN
		...
		v_table_nm := 'CMS_GROUP_PROG'  ;
  
    IF INSERTING THEN
      v_inst_code := :new.CGP_INST_CODE;
    ELSE
      v_inst_code := :old.CGP_INST_CODE;
    END IF;
...
P_DTL_INFO(1).COL_NAME := 'CGP_GROUP_CODE';
P_DTL_INFO(2).COL_NAME := 'CGP_PROG_CODE';
P_DTL_INFO(3).COL_NAME := 'CGP_INS_USER';
P_DTL_INFO(4).COL_NAME := 'CGP_INS_DATE';
P_DTL_INFO(5).COL_NAME := 'CGP_LUPD_USER';
P_DTL_INFO(6).COL_NAME := 'CGP_LUPD_DATE';
P_DTL_INFO(1).OLD_VAL := :OLD.CGP_GROUP_CODE;
P_DTL_INFO(2).OLD_VAL := :OLD.CGP_PROG_CODE;
P_DTL_INFO(3).OLD_VAL := :OLD.CGP_INS_USER;
P_DTL_INFO(4).OLD_VAL := :OLD.CGP_INS_DATE;
P_DTL_INFO(5).OLD_VAL := :OLD.CGP_LUPD_USER;
P_DTL_INFO(6).OLD_VAL := :OLD.CGP_LUPD_DATE;
P_DTL_INFO(1).NEW_VAL := :NEW.CGP_GROUP_CODE;
P_DTL_INFO(2).NEW_VAL := :NEW.CGP_PROG_CODE;
P_DTL_INFO(3).NEW_VAL := :NEW.CGP_INS_USER;
P_DTL_INFO(4).NEW_VAL := :NEW.CGP_INS_DATE;
P_DTL_INFO(5).NEW_VAL := :NEW.CGP_LUPD_USER;
P_DTL_INFO(6).NEW_VAL := :NEW.CGP_LUPD_DATE;
