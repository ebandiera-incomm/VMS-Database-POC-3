CREATE OR REPLACE TRIGGER VMSCMS.TR_BRANRFHGRP_GENAUDIT
AFTER  INSERT OR UPDATE OR DELETE ON VMSCMS.CMS_BRAN_REFRESHGRP FOR EACH ROW
DECLARE
 ------------------------------------------------------------------------------------------------
...
BEGIN
		...
		v_table_nm := 'CMS_BRAN_REFRESHGRP' ;
    
    IF INSERTING THEN
      v_inst_code :=:new.CBR_INST_CODE;
    ELSE
      v_inst_code := :old.CBR_INST_CODE;
    END IF;
...
P_DTL_INFO(1).COL_NAME := 'CBR_BRAN_CODE';
P_DTL_INFO(2).COL_NAME := 'CBR_RFRSH_GRP';
P_DTL_INFO(3).COL_NAME := 'CBR_INS_USER';
P_DTL_INFO(4).COL_NAME := 'CBR_INS_DATE';
P_DTL_INFO(5).COL_NAME := 'CBR_LUPD_USER';
P_DTL_INFO(6).COL_NAME := 'CBR_LUPD_DATE';


P_DTL_INFO(1).OLD_VAL := :OLD.CBR_BRAN_CODE;
P_DTL_INFO(2).OLD_VAL := :OLD.CBR_RFRSH_GRP;
P_DTL_INFO(3).OLD_VAL := :OLD.CBR_INS_USER;
P_DTL_INFO(4).OLD_VAL := :OLD.CBR_INS_DATE;
P_DTL_INFO(5).OLD_VAL := :OLD.CBR_LUPD_USER;
P_DTL_INFO(6).OLD_VAL := :OLD.CBR_LUPD_DATE;


P_DTL_INFO(1).NEW_VAL := :NEW.CBR_BRAN_CODE;
P_DTL_INFO(2).NEW_VAL := :NEW.CBR_RFRSH_GRP;
P_DTL_INFO(3).NEW_VAL := :NEW.CBR_INS_USER;
P_DTL_INFO(4).NEW_VAL := :NEW.CBR_INS_DATE;
P_DTL_INFO(5).NEW_VAL := :NEW.CBR_LUPD_USER;
P_DTL_INFO(6).NEW_VAL := :NEW.CBR_LUPD_DATE;
