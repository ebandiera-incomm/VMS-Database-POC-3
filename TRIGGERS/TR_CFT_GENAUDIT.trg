CREATE OR REPLACE TRIGGER VMSCMS.TR_CFT_GENAUDIT
AFTER  INSERT OR UPDATE OR DELETE ON  CMS_FEE_TYPES FOR EACH ROW
DECLARE
 ------------------------------------------------------------------------------------------------
...
BEGIN
        ...
        v_table_nm := 'CMS_FEE_TYPES'  ;
        
        IF INSERTING THEN
          v_inst_code := :new.CFT_INST_CODE;
        ELSE
          v_inst_code := :old.CFT_INST_CODE;
        END IF;
...
P_DTL_INFO(1).COL_NAME := 'CFT_FEETYPE_CODE';  -- NOT USED
P_DTL_INFO(2).COL_NAME := 'CFT_FEETYPE_DESC';
P_DTL_INFO(3).COL_NAME := 'CFT_FEE_FREQ';
P_DTL_INFO(4).COL_NAME := 'CFT_INS_USER';
P_DTL_INFO(5).COL_NAME := 'CFT_INS_DATE';
P_DTL_INFO(6).COL_NAME := 'CFT_LUPD_USER';
P_DTL_INFO(7).COL_NAME := 'CFT_LUPD_DATE';


P_DTL_INFO(1).OLD_VAL := :OLD.CFT_FEETYPE_CODE;
P_DTL_INFO(2).OLD_VAL := :OLD.CFT_FEETYPE_DESC;
P_DTL_INFO(3).OLD_VAL := :OLD.CFT_FEE_FREQ;
P_DTL_INFO(4).OLD_VAL := :OLD.CFT_INS_USER;
P_DTL_INFO(5).OLD_VAL := :OLD.CFT_INS_DATE;
P_DTL_INFO(6).OLD_VAL := :OLD.CFT_LUPD_USER;
P_DTL_INFO(7).OLD_VAL := :OLD.CFT_LUPD_DATE;


P_DTL_INFO(1).NEW_VAL := :NEW.CFT_FEETYPE_CODE;
P_DTL_INFO(2).NEW_VAL := :NEW.CFT_FEETYPE_DESC;
P_DTL_INFO(3).NEW_VAL := :NEW.CFT_FEE_FREQ;
P_DTL_INFO(4).NEW_VAL := :NEW.CFT_INS_USER;
P_DTL_INFO(5).NEW_VAL := :NEW.CFT_INS_DATE;
P_DTL_INFO(6).NEW_VAL := :NEW.CFT_LUPD_USER;
P_DTL_INFO(7).NEW_VAL := :NEW.CFT_LUPD_DATE;
