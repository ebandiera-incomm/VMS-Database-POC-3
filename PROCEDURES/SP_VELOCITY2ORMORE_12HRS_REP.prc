CREATE OR REPLACE PROCEDURE vmscms.sp_velocity2ormore_12hrs_rep (
   frmdt        IN       VARCHAR2,
   todt         IN       VARCHAR2,
...
IS
   ...
   IS
      SELECT chr_row_id, chr_primary_card_number, chr_txn_date
        FROM cms_velocityload_txn12hrs;
       ...
       WHERE chr_primary_card_number = pan
         AND TO_DATE (chr_txn_date, 'yyyymmddhh24miss')
                BETWEEN TO_DATE (txndt, 'yyyymmddhh24miss')
                    AND TO_DATE (txndt, 'yyyymmddhh24miss') + 0.5;

   CURSOR loadtmp2
   IS
      SELECT DISTINCT chr_row_id
                 FROM cms_velocityall_txn12hrs;
   ...
   IS
      SELECT   x.cardno primary_card_number,
          ...
          FROM (SELECT fn_dmaps_main (cap_pan_code_encr) cardno, cap_pan_code,
                 ...
                 WHERE a.cap_inst_code = b.cam_inst_code
                   AND a.cap_bill_addr = b.cam_addr_code) x,
               (SELECT a.customer_card_no, terminal_id, merchant_name,
                       ...
                       (SELECT DISTINCT cms_resp_desc
                                  ...
                                  WHERE cms_inst_code =
                                                       instcode
                                    AND cms_iso_respcde = response_code
                 ...
                 WHERE instcode = 1
                   ...
                   AND 2 <
                          (SELECT   COUNT (*)
                               FROM transactionlog
                              WHERE instcode = 1
                                AND customer_card_no = a.customer_card_no
                                AND (   (    delivery_channel = '04'
                                         AND txn_code IN
                                                ('68', '69', '80', '82', '85',
                                                 '88')
                           ...
                           GROUP BY customer_card_no)) y
         WHERE x.cap_pan_code = y.customer_card_no
      ORDER BY x.cardno, txndt;
BEGIN
   prm_errmsg := 'OK';

    truncate_tab_ebr ('CMS_VELOCITY_TXN12HRS');
    truncate_tab_ebr ('CMS_VELOCITYLOAD_TXN12HRS');
    truncate_tab_ebr ('CMS_VELOCITYALL_TXN12HRS');

   BEGIN
      FOR i IN LOAD
      LOOP
         INSERT INTO cms_velocityload_txn12hrs
          ...
          WHERE chr_row_id = j.chr_row_id
            ...
            AND TO_DATE (chr_txn_date, 'yyyymmddhh24miss')
                   BETWEEN TO_DATE (j.chr_txn_date, 'yyyymmddhh24miss')
                       AND TO_DATE (j.chr_txn_date, 'yyyymmddhh24miss') + 0.5;
         ...
         THEN
            SELECT COUNT (*)
             ...
             WHERE chr_primary_card_number = j.chr_primary_card_number
               AND TO_DATE (chr_txn_date, 'yyyymmddhh24miss')
                      BETWEEN TO_DATE (j.chr_txn_date, 'yyyymmddhh24miss')
                          AND   TO_DATE (j.chr_txn_date, 'yyyymmddhh24miss')
                              + 0.5;
            ...
            THEN
               ...
               LOOP
                  INSERT INTO cms_velocityall_txn12hrs
   ...
   BEGIN
      ...
      LOOP
         ...
         LOOP
            INSERT INTO cms_velocity_txn12hrs
