CREATE OR REPLACE PROCEDURE VMSCMS.sp_gen_reissuepan_cms_271110 (
...
/*************************************************
     ...
     * PURPOSE             : Regenerate new pan number on the basis of old pan
     * Modified By:        : Chinmaya Behera	
     * Modified Date       : 27-05-2010
   ***********************************************/
   ...
   IS
      SELECT cpa_acct_id, cpa_acct_posn
        FROM cms_pan_acct
       WHERE cpa_pan_code = prm_pancode
...
---------------------------------------En find hsm mode-------------------------------------
   ...
   BEGIN                            --Begin 1 Block Starts Here       --    B3
      SELECT cap_inst_code, cap_asso_code, cap_inst_type, cap_prod_code,
             ...
             cap_online_aggr_limit, cap_firsttime_topup, cap_mbr_numb,
			 type_appl_rec_array
						(
			 	  cap_panmast_param1,
				  cap_panmast_param2,  
        ...
        INTO v_inst_code, v_asso_code, v_inst_type, v_prod_code,
             ...
             v_online_aggr_limit, v_cap_firsttime_topup,v_mbrnumb,
			 v_appl_data
        FROM cms_appl_pan
       WHERE cap_pan_code = prm_pancode AND cap_inst_code = prm_instcode;
   EXCEPTION
  ...
  END IF;
   ...
   EXCEPTION
		 ...
		 v_errmsg :=
               'Product is not related to cardtype';
                RAISE exp_reject_record;
		WHEN OTHERS THEN
		v_errmsg :=
               'Error while selecting product and cardtype relationship' || substr(sqlerrm,1,200);
                RAISE exp_reject_record;
...
--                 RAISE exp_reject_record;
		WHEN OTHERS THEN
		v_errmsg :=
               'Error while selecting product and custcatg relationship' || substr(sqlerrm,1,200);
                RAISE exp_reject_record;
      END;		
...
--DBMS_OUTPUT.put_line ('SERIAL No ' || v_serial_no);
  ...
  -- DBMS_OUTPUT.put_line (v_tmp_pan);
   /*  FOR i IN 1 .. v_loop_max_cnt
      ...
      END LOOP;*/---------------------------------------Sn generate temp pan for check digit-------------------------------------
   v_tmp_pan := NULL;
   FOR i IN 1 .. v_loop_max_cnt
   LOOP
      FOR j IN 1 .. v_loop_max_cnt
   ...
   BEGIN
      --  dbms_output.put_line('test Main test for error 34511');
      --DBMS_OUTPUT.put_line ('test Main pan' || v_pan);               --    B8
   ...
   EXCEPTION
      ...
      THEN
         v_errmsg :=
                   'Error while fetching data for pan code ' || prm_pancode;
         RAISE exp_reject_record;
      WHEN NO_DATA_FOUND
      THEN
         v_errmsg :=
...
--         FROM DUAL;
   ...
   END;
            --En get acct number
   ---------------------------------------Sn Create Account for a new pan-------------------------------------
   IF v_errmsg = 'OK'
   THEN
      BEGIN
                                           --  B9.1
         ...
         IF v_errmsg != 'OK'
         THEN
            v_errmsg := 'Problem while calling the sp_ceate_acct' || SQLERRM;
            RAISE exp_reject_record;
         END IF;
      EXCEPTION
	 WHEN EXP_REJECT_RECORD THEN
	 RAISE;
         ...
         THEN
            v_errmsg :=
                      'Problem while creating acct' || SQLERRM;
            RAISE exp_reject_record;
      END;                                                           --   B9.1
   END IF;
   IF v_errmsg = 'OK'
   THEN
      BEGIN
         -- dbms_output.put_line('test Main test for error 345111');                                                --    B9.2
...
--*******************************************************************************
   END IF;
   ---------------------------------------En Create Account for a new pan-------------------------------------
   ...
   THEN
      BEGIN
         ...
         SELECT cap_addon_link
           ...
           FROM cms_appl_pan
          WHERE cap_appl_code = prm_pancode
      ...
      EXCEPTION                                                     --excp 1.1
         ...
         WHEN OTHERS
         THEN
            v_errmsg := 'Error while selecting addon detail ' || substr(SQLERRM,1,150);
            RAISE exp_reject_record;
   ...
   WHEN OTHERS THEN
	   ...
	   RAISE exp_reject_record;
	    ...
	    RAISE exp_reject_record;
   END;
   --En find card status and limit parameter for the profile                                                     --  B11
...
/*
 ...
 v_expry_date := LAST_DAY (ADD_MONTHS (SYSDATE, v_expryparam - 1));
   ...
   BEGIN
	  ...
	  IF v_cpm_catg_code =  'P' THEN                                               --  B17
      INSERT INTO cms_appl_pan
                  (cap_inst_code, cap_asso_code, cap_inst_type,
                   cap_prod_code, cap_prod_catg, cap_card_type,
                   cap_cust_catg, cap_pan_code, cap_mbr_numb, cap_card_stat,
                   cap_cust_code, cap_disp_name, cap_limit_amt,
                   ...
                   cap_next_bill_date, cap_ins_user, cap_lupd_user,
                   cap_pbfgen_flag, cap_next_mb_date, cap_atm_offline_limit,
                   cap_atm_online_limit, cap_pos_offline_limit,
                   cap_pos_online_limit, cap_offline_aggr_limit,
           ...
           VALUES (prm_instcode, v_asso_code, v_inst_type,
                   ...
                   v_cust_catg, v_pan, v_mbrnumb, v_card_stat,
                   v_cust_code, v_disp_name, v_limit_amt,
                   v_use_limit, v_appl_bran, SYSDATE,
       --v_active_date,  Commented as Active Date for New should go as Sysdate
                   v_expry_date, v_addon_stat, v_adonlink,
                   v_mbrlink, v_acctid_new,
                     --v_acct_id, Commented as AccountID should be of New Card
                                           v_pan, v_tot_acct,
                   v_bill_addr, v_chnl_code, SYSDATE,
                   prm_lupduser, 'Y', v_pingen_flag,
                   v_emboss_flag, 'N', 'N',
                   NULL, prm_lupduser, prm_lupduser,
                   'R', v_next_mb_date, v_offline_atm_limit,
                   v_online_atm_limit, v_offline_pos_limit,
                   v_online_pos_limit, v_offline_aggr_limit,
			      ...
			      BEGIN
			         ...
			         INSERT INTO cms_pan_acct
			              ...
			              VALUES (prm_instcode, v_cust_code, x.cpa_acct_id,
			                      x.cpa_acct_posn, v_pan, v_mbrnumb,
	  	 ...
	  	 IF v_cpm_catg_code =  'P' THEN
		         ...
		         THEN
		            UPDATE cms_acct_mast
		             ...
		             WHERE cam_inst_code = prm_instcode AND cam_acct_id = v_acctid_new;
		         ELSE
		            UPDATE cms_acct_mast
		               SET cam_acct_no = v_pan,
