CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_ROLL_ACCT_DATA_SUMRY_REP
AS
   ...
   IS
     select (select substr(MFI_FILE_NAME,1,instr(MFI_FILE_NAME,'_ACCO')-1)
        ...
        where mad_acct_numb in
        (
        select mci_seg31_num
        from migr_caf_info_entry
        where mci_proc_flag='S'
        and mci_migr_seqno =prm_seqno
        ))+(select count(1)
        from migr_acct_data_temp
        where mad_acct_numb in
        (
        select MCI_SAVIGS_ACCT_NUMBER
        from migr_caf_info_entry
        where mci_proc_flag='S'
        and mci_migr_seqno =prm_seqno
        ))) RECORDS_MIGRATED,
       (select MRC_DEL_CNT from vmscms.MIGR_ROLL_COUNT
       where MRC_TABLE_NAME = 'CMS_ACCT_MAST' and MRC_MIGR_SEQNO =prm_seqno
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'ROLL_ACCT_DATA_SUMRY_REP.csv';
