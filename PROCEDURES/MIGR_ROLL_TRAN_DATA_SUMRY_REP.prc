CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_ROLL_TRAN_DATA_SUMRY_REP 
AS
   ...
   IS
    ...
    where Mtt_MIGR_SEQNO = PRM_SEQNO and mtt_flag='S') RECORDS_MIGRATED,
    (select MRC_DEL_CNT from vmscms.MIGR_ROLL_COUNT 
     where MRC_TABLE_NAME = 'TRANSACTIONLOG' and MRC_MIGR_SEQNO =PRM_SEQNO
    ...
    (select MRC_DEL_CNT from vmscms.MIGR_ROLL_COUNT 
     ...
     (
      ...
      where MRC_TABLE_NAME = 'TRANSACTIONLOG' and MRC_MIGR_SEQNO =PRM_SEQNO) - 
      (select count(*) from VMSCMS.MIGR_TRANSACTIONLOG_TEMP
      where mtt_MIGR_SEQNO = PRM_SEQNO and mtt_flag='S'
      ) 
     )
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'ROLL_TRAN_DATA_SUMRY_REP.csv';
