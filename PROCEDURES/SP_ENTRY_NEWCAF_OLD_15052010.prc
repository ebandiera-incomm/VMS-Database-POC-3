CREATE OR REPLACE PROCEDURE VMSCMS.Sp_Entry_Newcaf_old_15052010 (	instcode	IN		NUMBER		,
				lupduser	IN		NUMBER		,
				errmsg		OUT		VARCHAR2	)
AS
...
dbms_output.put_line('Creating new account...');
		Sp_Create_Acct(instcode, v_cci_seg31_num ,1, branch, addr, v_cat_type_code, v_cas_stat_code, lupduser, acctid, lperr)	;
		IF	lperr !=  'OK' THEN
		...
		IF errmsg = 'OK' THEN
			...
			--				dum := 0;
				BEGIN		--begin 1.1 --customer part
					...
					UPDATE CMS_CAF_INFO_ENTRY SET cci_approved = 'E' WHERE cci_row_id = y.cci_row_id AND cci_ins_date = y.cci_ins_date;
					END IF;
				EXCEPTION		--excp 1.1
					WHEN OTHERS THEN
				...
				IF errmsg = 'OK' THEN	--address part
					...
					y.cci_seg12_open_text1,NULL,v_gcm_cntry_code,y.cci_seg12_city,y.cci_seg12_state,NULL,'P',lupduser,addrcode,errmsg);
				...
				IF	errmsg = 'OK' THEN	--application part
					BEGIN		--begin 1.4
						...
						AND	cpm_prod_b24	=		DECODE(trim(y.cci_crd_typ),'VD','VD','ND','PD','NA','P','MD','MD');
						...
						IF y.cci_seg12_branch_num ='*' OR y.cci_seg12_branch_num IS NULL THEN	--cust catg comes as '*   ' or '    ' in the infile
