CREATE OR REPLACE PROCEDURE VMSCMS.sp_calc_fees_old(	instcode	IN	NUMBER	,
...
/*
	...
	WHERE			cap_inst_code	=	instcode
		...
		AND			cap_card_type	=	cardtype
		AND			cap_cust_catg	=	custcatg
...
--	DBMS_OUTPUT.PUT_LINE('ANNUAL '||V_CHECK_CTRL);
	...
	IF v_check_ctrl > 0 THEN
	   ...
	   DBMS_OUTPUT.PUT_LINE('BEFORE INSERTING INTO CHARGE DTL for date :' || calcdate);
		...
		AND			a.cap_card_type	=	cardtype
		AND			a.cap_cust_catg	=	custcatg
		AND			calcdate  >= b.cpf_vfrom AND calcdate <= b.cpf_valid_to
		AND			TRUNC(a.cap_next_bill_date)	=	TRUNC(calcdate)
		AND			b.cpf_fee_code	=	c.cfm_fee_code
		AND			b.cpf_fee_code	=	feecode
		AND			c.cfm_feetype_code =	d.cft_feetype_code
...
--	    DBMS_OUTPUT.PUT_LINE('COMPLETED INSERTION INTO CHARGE DTL');
		...
		IF SQL%rowcount > 0 THEN
				  ...
				  WHERE	cap_pan_code IN (SELECT	ccd_pan_code
		 						 	 	 FROM	CMS_CHARGE_DTL
...
BEGIN		--begin lp1
dbms_output.put_line('Starting the calc_on_pccc procedure now...');
  lperr1 := 'OK';

  FOR x IN lp1c1
  LOOP
  dbms_output.put_line('LOOP START -->lp_calc_on_pccc');
	IF	lperr1 != 'OK' THEN
		EXIT;
	END IF;
	...
	dbms_output.put_line(' FEE FREQUENCY --> '||v_cfm_fee_freq);
	dbms_output.put_line(' FEE AMOUNT --> '||v_cfm_fee_AMT);
...
*/
	...
	BEGIN     --begin lp1.1
		...
		dbms_output.put_line (v_cpw_waiv_prcnt);

		waivamt	:= ( v_cpw_waiv_prcnt / 100 ) * v_cfm_fee_amt;

		dbms_output.put_line (waivamt);

		lperr1 := 'OK';
		dbms_output.put_line (LPERR1||'1');
...
dbms_output.put_line ('ERROR lp_calc_on_pccc' || lperr1);
	...
	IF lperr1 = 'OK' THEN
		...
		ELSIF v_cfm_fee_freq = 'A' THEN
			  dbms_output.put_line('b4 lp_attchfee_for_annual values are as follows:');
dbms_output.put_line('card type is :' ||x.cpf_card_type ||	' cust catg is:' || x.cpf_cust_catg)	;
...
--call the local procedure which selects the parameter which speecifies the no of days after which the account should be hit for debiting the fees
  	...
  	lp_select_param_value;
			...
			dbms_output.put_line('INITIAL RUNFORDATE -->'||calcdate);

			dbms_output.put_line('SELECT IN MAIN -->'|| errmsg);
...
--  dbms_output.put_line('level1');
  		...
  		IF errmsg = 'OK'  THEN --if a
	 	   ...
	 	   dbms_output.put_line('NUMBER OF DAYS -->'||datecnt);
...
-- 			errmsg := 'Procedure has already been run successfully on '||sysdate;
--		ELSE
--			calcdate := trunc(sysdate);   ---CHANGE PROPOSED TO ENTER CALCDATE = SYSDDATE-DATECNT
	...
	ELSE         -------b
		FOR i IN 1..datecnt
		LOOP
		dbms_output.put_line('Main LOOP START NO' || i );
		dbms_output.put_line('ERRMSG -->'|| ERRMSG);
			...
			calcdate := calcdate+1;
			dbms_output.put_line('Starting new stuff for DATE --> '||calcdate);
...
dbms_output.put_line('Error before insert proc_ctrl--->'|| errmsg);
			INSERT INTO CMS_PROC_CTRL (	CPC_PROC_NAME		,
						...
						VALUES(		'FEE CALC'	,
								...
								calcdate	);
			IF SQL%rowcount  = 0 THEN
			   dbms_output.put_line('No rows inserted into proc ctrl for calcdate-->'|| calcdate);
            END IF;

			END IF;       --------g
  dbms_output.put_line('Error message for the date '|| calcdate || 'is ' || errmsg);
		END LOOP;
