CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_CUST_CARD_DATA_SUMRY_REP 
AS
   ...
   IS
    ...
    (select count(*) from VMSCMS.migr_caf_info_entry where MCI_FILE_NAME = prm_file_name) Total_Number_of_Records ,
    (select count(*) from VMSCMS.migr_caf_info_entry
    where mci_proc_flag='S' and MCI_FILE_NAME = prm_file_name) Records_Migrated,
    (select count(*) from VMSCMS.migr_caf_info_entry
    where mci_proc_flag='E' and MCI_FILE_NAME = prm_file_name) Records_Rejected
    From dual;
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'CUST_CARD_DATA_SUMRY_REP.csv';
