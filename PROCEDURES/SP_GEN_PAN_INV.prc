CREATE OR REPLACE PROCEDURE VMSCMS.SP_GEN_PAN_INV(PRM_INSTCODE IN NUMBER,
                                PRM_APPLCODE IN NUMBER,
                                --                       PRM_IP_ADDR         IN VARCHAR2, /* T.NARAYANAN. ADDED - AUDIT LOG REPORT */
                                PRM_LUPDUSER        IN NUMBER,
                                PRM_PAN             OUT VARCHAR2,
                                PRM_ENCR_PAN        OUT VARCHAR2, --added on 14-Jun-2012
   ...
   /*************************************************
       ...
       * PURPOSE          :  pan generation for inventory
       * Modified By      :
       * Modified Date    :
       * Modified Reason  :
       * Reviewer         :
       * Reviewed Date    :
       * Build  No.       : RI0010_B0004
   *************************************************/
...
BEGIN
  ...
  IF V_CPM_CATG_CODE = 'D' OR V_CPM_CATG_CODE = 'A' THEN
    SP_GEN_PAN_DEBIT_INV(PRM_INSTCODE,
                    PRM_APPLCODE,
                    PRM_LUPDUSER,
  ...
  ELSIF V_CPM_CATG_CODE = 'P' THEN
    SP_GEN_PAN_PREPAID_INV(PRM_INSTCODE,
                      PRM_APPLCODE,
                      PRM_LUPDUSER,
                      --              PRM_IP_ADDR, /* T.NARAYANAN. ADDED - AUDIT LOG REPORT */
                      PRM_PAN,
