CREATE OR REPLACE PROCEDURE VMSCMS.sp_blockpan_support_log
  (
...
--EN create encr pan
 ...
 BEGIN
  insert into CMS_BLOCK_DETAIL(CBD_INST_CODE,CBD_CARD_NO,CBD_FILE_NAME,
                         ...
                         v_hash_pan, prm_file_name,
                                prm_remarks, prm_msg24_flag,prm_processs_flag,
                                prm_process_msg, prm_process_mode, prm_ins_user,
                                prm_ins_date, prm_ins_user, prm_ins_date,v_encr_pan
                               );
--En create record u
 EXCEPTION
 WHEN OTHERS THEN
 prm_errmsg := 'Error while creating record in Block pin detail table ' || substr(sqlerrm,1,150);
 RETURN;
 END;
