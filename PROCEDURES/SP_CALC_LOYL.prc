CREATE OR REPLACE PROCEDURE VMSCMS.sp_calc_loyl (
   instcode   IN       NUMBER,
   lupduser   IN       NUMBER,
   errmsg     OUT      VARCHAR2
)
...
--main cursor which picks up the transactions to calculate loyalty points
   ...
   IS
      SELECT   ROWID, cpt_id_col, cpt_auth_code, cpt_trans_type,
		 ...
		 --AND cpt_pan_code ='3149990300000195'
           AND cpt_rec_typ = '02'
...
--this means that the transaction has been carried out in the month for which a loyalty is defined
            ...
            IF    excp_temp_loyl_code IS NOT NULL
               OR pccc_temp_loyl_code IS NOT NULL
            THEN                                                        ---111
               IF    excp_temp_loyl_point IS NOT NULL
                  OR pccc_temp_loyl_point IS NOT NULL
               THEN                                                     ---222
                  IF    excp_temp_trans_amt IS NOT NULL
                     OR pccc_temp_trans_amt IS NOT NULL
                  THEN                                                  ---333
                     ...
                     THEN
                        ...
                        EXCEPTION
                           ...
                           THEN
                              errmsg :=
                                 'No data found for the loyalty in loyalty mast';
                              RAISE v_raise_exception;
                           WHEN OTHERS
                           THEN
                              errmsg :=
                     ...
                     THEN
                        ...
                        IF slab_flag != 'Y'
                        THEN
                           SELECT   ROUND (  NVL (x.cpt_trans_amt, 0)
                                           / applicable_trans_amt
                                          )
                        ...
                        THEN                                  --errmsg = ok if
                           ...
                           BEGIN
                              ...
                              UPDATE cms_loyl_points
                                 SET clp_loyl_points =
                                            clp_loyl_points + calc_loyl_points,
                              ...
                              THEN
                                 BEGIN
                                    ...
                                    THEN
                                       errmsg :=
                                          'error while insertind record in loyl_audit';
                                       RAISE v_raise_exception;
                                    END IF;
                                 EXCEPTION
                              ...
                              BEGIN
                                 INSERT INTO cms_loyl_dtl
                                             (cld_inst_code, cld_pan_code,
                                              cld_mbr_numb, cld_acct_no,
                                              cld_loyl_code,
                                              cld_trans_amt,
                                              cld_loyl_points,
                                              cld_addon_pan, cld_addon_mbr,
                                              cld_id_col,
                                              cld_tran_date, cld_ins_user,
                                              cld_lupd_user
                                             )
                                      VALUES (instcode, addon_for_insert,
                                              mbr_for_addon, x.cpt_acct_no,
                                              applicable_loyl_code,
                                              x.cpt_trans_amt,
                                              calc_loyl_points,
                                              excp_addon, excp_mbr,
                                              x.cpt_id_col,
                                              x.cpt_trans_date, lupduser,
                                              lupduser
                                             );
                                 ...
                                 THEN
                                    errmsg :=
                                       'error while insertind record in loyl_detail';
                                    RAISE v_raise_exception;
                                 END IF;
                              EXCEPTION
                                 WHEN v_raise_exception
                              ...
                              BEGIN
                                 INSERT INTO cms_loyl_audit
                                             (cla_inst_code, cla_pan_code,
                                              cla_mbr_numb, cla_acct_no,
                                              cla_loyl_ind, cla_loyl_points,
                                              cla_oprn_date,
                                              cla_oprn_desc,
            ...
            IF excp_temp_loyl_code IS NULL AND pccc_temp_loyl_code IS NULL
            THEN
               errmsg := 'No loyalty for record with PAN :' || x.cpt_pan_code;
               RAISE v_raise_exception;
            END IF;
            ...
            THEN
               errmsg :=
                   'No Loyalty Point for record with PAN :' || x.cpt_pan_code;
               RAISE v_raise_exception;
            END IF;
            ...
            THEN
               errmsg :=
                  'No Transaction Amt for record with PAN :'
                  || x.cpt_pan_code;
               RAISE v_raise_exception;
            END IF;
            ...
            BEGIN
	            ...
	            THEN
	               errmsg :=
	                  'error while updating record in pan trans';
	               RAISE v_raise_exception;
	            END IF;
	         EXCEPTION
	            WHEN v_raise_exception
   ...
   EXCEPTION                                                          --excp 1
      WHEN OTHERS
      THEN
         errmsg := 'Out main Excp 1 -- ' || errmsg || SQLERRM;
   END;                                                           --end begin1
EXCEPTION                                                 --Excp of main begin
   WHEN OTHERS
   THEN
      errmsg := 'Main Excp -- ' || errmsg || SQLERRM;
END;                                                         --Main begin ends
/
