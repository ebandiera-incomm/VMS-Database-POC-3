CREATE OR REPLACE PROCEDURE VMSCMS.sp_login_checks_siva
  (
    instcode IN NUMBER,
...
BEGIN --main begin
  ...
  SELECT cum_user_susp,
    ...
    cum_valid_frdt,
    cum_valid_todt,
    TO_CHAR (NVL (TO_DATE (cum_last_logintime, 'dd-mon-yyyy hh24:mi:ss'), SYSDATE ), dateformate
    || ' HH24:MI:SS' ),
    cum_force_pswd
  ...
  IF v_adauth_flag='N' THEN
      ...
      IF loginpswd       != v_cum_encr_pswd THEN
        ...
        IF NOT (UPPER (v_cug_group_name) = UPPER ('SUPER USER')) THEN
          IF dum                        >= v_wrong_logins THEN
            ...
            WHERE cum_inst_code = instcode
            AND cum_user_pin    = userpin;
          ELSIF dum             = v_wrong_logins - 1 THEN
   ...
   IF v_cum_user_susp='L' THEN
   BEGIN
   select to_number(to_char(sysdate,'MI')) - to_number(to_char(CUM_LUPD_DATE,'MI'))      
      INTO v_time_diff      
      from cms_user_mast
      where cum_inst_code = instcode
      AND cum_user_pin = userpin;
  ...
  IF errmsg = 'OK' THEN --ok if
    ...
    ELSIF v_cum_user_susp = 'D' THEN
      ...
      flagint            := 1;
    END IF;
    IF TRUNC (SYSDATE) NOT BETWEEN TRUNC (v_cum_valid_frdt) AND TRUNC (v_cum_valid_todt) THEN
