CREATE OR REPLACE PROCEDURE VMSCMS.migr_find_binprefix (
   p_instcode    IN       NUMBER,
   p_prod_code   IN       VARCHAR2,
   p_card_type            NUMBER,
   p_branch      IN       VARCHAR2,
   --p_lupduser    IN       NUMBER,  --commented by Pankaj S. as its unused
   p_tmp_pan     OUT      VARCHAR2,
   p_max_serl    OUT      VARCHAR2,
...
BEGIN
   ...
   EXCEPTION
      ...
      THEN
         v_errmsg :=
               'Error while selecting Profile details ' --Error message modified by Pankaj S. on 25-Sep-2013
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
         v_errmsg :='No prefix  found for combination of Institution '  --Error message modified by Pankaj S. on 25-Sep-2013
            || p_instcode
            || ' and product '
            || p_prod_code;
      WHEN OTHERS
      THEN
         v_errmsg := 'Error while selecting BIN prefix -- ' || SQLERRM; --Error message modified by Pankaj S. on 25-Sep-2013
   END;
   ...
   BEGIN
      ...
      LOOP
         ...
         DBMS_OUTPUT.put_line (   'RRRRRRRRRRRR-----'
                               || v_table_pan_construct (i).cpc_field_name
                               || ' Field VAlue --'
                               || v_table_pan_construct (i).cpc_field_value
                              );
   ...
   EXCEPTION
      WHEN OTHERS
      THEN
         v_errmsg := 'Error from pan construct process ' || SUBSTR (SQLERRM, 1, 300); --Error message modified by Pankaj S. on 25-Sep-2013
         RETURN;
   END;
   ...
   LOOP
      ...
      LOOP
         ...
         THEN
            p_tmp_pan :=
                       p_tmp_pan || v_table_pan_construct (j).cpc_field_value;
            DBMS_OUTPUT.put_line ('OOOOOOOOOO-----' || p_tmp_pan);
            EXIT;
         END IF;
      END LOOP;
   END LOOP;
