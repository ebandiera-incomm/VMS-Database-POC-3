CREATE OR REPLACE PROCEDURE VMSCMS.sp_proxy_shuffler_test
( PRM_PROD_CODE VARCHAR2 ,
  PRM_CARD_TYPE NUMBER ,
  PRM_RANGE  NUMBER
  )
IS
   CURSOR c_orig_rec ( v_frm_range NUMBER  ,v_to_range NUMBER  )
   IS
      SELECT ROWID row_id, cap_proxy_number
        FROM cms_appl_pan
         WHERE CAP_PROD_CODE=PRM_PROD_CODE and CAP_CARD_TYPE=PRM_CARD_TYPE
          and cap_proxy_number >= v_frm_range and cap_proxy_number <=v_to_range ;
