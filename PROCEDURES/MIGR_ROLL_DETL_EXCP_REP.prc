CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_ROLL_DETL_EXCP_REP
AS
   ...
   IS     
    ...
    MDR_FILE_NAME FILE_NAME,
    MDR_REASON REASON,
    TO_CHAR(MDR_INS_DATE,'DD-MON-YYYY') INS_DATE
    from migr_det_roll_excp 
    where MDR_MIGR_SEQNO = prm_seqno;
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'MIGR_ROLL_DETL_EXCP_REP.csv';
         ...
         THEN
            BEGIN
               l_total_records := l_total_records + 1;
                   ...
                   UTL_FILE.put (l_file, cur_data.REASON || ',');
                   UTL_FILE.put (l_file, cur_data.INS_DATE || ',');
