CREATE OR REPLACE PROCEDURE VMSCMS.Sp_Unblock_Pan_Prepaid (
...
AS
   ...
   v_deblock_auth_id        TRANSACTIONLOG.auth_id%TYPE;
...
*/
   ...
   BEGIN
      SELECT cfm_txn_code, cfm_txn_mode, cfm_delivery_channel, cfm_txn_type
        ...
        FROM CMS_FUNC_MAST
       WHERE cfm_func_code = 'UNBLOKSPRT' AND cfm_inst_code=prm_instcode;
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
   ...
   --Sn For Debit No need  of authorization
   ...
   BEGIN
      UPDATE CMS_APPL_PAN
       ...
       AND cap_inst_code=prm_instcode; --prm_acctno;
   ...
   BEGIN
      INSERT INTO CMS_PAN_SPPRT
           ...
           v_hash_pan, v_mbrnumb, v_cap_prod_catg,
                   'UNBLOCK', prm_resoncode, prm_remrk,
