CREATE OR REPLACE PROCEDURE VMSCMS.sp_ins_base2ilf_mcrncy(errmsg	OUT		VARCHAR2)
AS
C_RBR_RECON NUMBER(1):= 5;
BEGIN
	ERRMSG:= 'OK';

   UPDATE REC_BASE2_ILF_TEMP_MCRNCY SET rbi_crd_iss_cntry = fn_get_CrdIssuing_Id(SUBSTR(Rbi_PAN,1,6));

--Imran 22 Aug 2003
	UPDATE REC_BASE2_ILF_TEMP_MCRNCY SET rbi_pan=trim(rbi_pan),rbi_resp_cde=trim(rbi_resp_cde);
--imran 28 May 03
--	INSERT INTO REC_BASE2_ILF (SELECT * FROM REC_BASE2_ILF_TEMP where rbi_resp in('000','001') AND RBI_TRAN_DAT<>'000000');
DELETE FROM REC_BASE2_ILF_TEMP_MCRNCY WHERE RBI_TRAN_DAT='000000';
UPDATE REC_BASE2_ILF_TEMP_MCRNCY SET rbi_dup_flag='Y' WHERE RBI_PAN||RBI_RESP_CDE||RBI_SEQ_NUM||RBI_TRAN_DAT||RBI_REC_TYP||RBI_RVSL_CDE
            IN (SELECT RBI_PAN||RBI_RESP_CDE||RBI_SEQ_NUM||RBI_TRAN_DAT||RBI_REC_TYP||RBI_RVSL_CDE FROM REC_BASE2_ILF_MCRNCY);
	INSERT INTO REC_BASE2_ILF_MCRNCY (SELECT * FROM REC_BASE2_ILF_TEMP_MCRNCY WHERE rbi_dup_flag='N' AND rbi_resp IN ('000','001'));
--Imran 23 Jun 2003
	...
	INSERT INTO REC_BASE2_RECON_TEMP_MCRNCY	(
		...
		RBR_MAT_CAT_CODE , RBR_CRD_ISS_CNTRY )
	SELECT  C_RBR_RECON,		SYSDATE,		A.RBI_FILE_TYPE,	A.RBI_FILE_NAME,	A.RBI_REC_TYP,
		...
		A.RBI_MAT_CAT_CODE, A.RBI_CRD_ISS_CNTRY
		FROM	REC_BASE2_ILF_TEMP_MCRNCY A
