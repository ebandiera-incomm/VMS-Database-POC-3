CREATE OR REPLACE PROCEDURE VMSCMS.sp_ins_base2ilf_sms(errmsg	OUT		VARCHAR2)
AS
C_RBR_RECON NUMBER(1):= 5;
BEGIN
	ERRMSG:= 'OK';
--Imran 22 Aug 2003
	UPDATE REC_BASE2_ILF_SMSTEMP SET rbi_pan=trim(rbi_pan),rbi_resp_cde=trim(rbi_resp_cde);
--imran 28 May 03
--	INSERT INTO REC_BASE2_ILF (SELECT * FROM REC_BASE2_ILF_SMSTEMP where rbi_resp in('000','001') AND RBI_TRAN_DAT<>'000000');
DELETE FROM REC_BASE2_ILF_SMSTEMP WHERE RBI_TRAN_DAT='000000';
UPDATE REC_BASE2_ILF_SMSTEMP SET rbi_dup_flag='Y' WHERE rbi_tran_typ NOT IN ('0220','0422') AND RBI_PAN||RBI_RESP_CDE||RBI_SEQ_NUM||RBI_TRAN_DAT||RBI_REC_TYP||RBI_RVSL_CDE    IN (SELECT RBI_PAN||RBI_RESP_CDE||RBI_SEQ_NUM||RBI_TRAN_DAT||RBI_REC_TYP||RBI_RVSL_CDE FROM REC_BASE2_ILF);
	INSERT INTO REC_BASE2_ILF (SELECT * FROM REC_BASE2_ILF_SMSTEMP WHERE rbi_dup_flag='N' AND rbi_resp IN ('000','001') AND rbi_tran_typ NOT IN ('0220','0422'));
--Imran 23 Jun 2003
