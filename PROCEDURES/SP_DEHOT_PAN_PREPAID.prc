CREATE OR REPLACE PROCEDURE VMSCMS.sp_dehot_pan_prepaid (
...
/*************************************************
     ...
     * PURPOSE          : DeHot List card for a perticular card number
     * Modified By:    :
     * Modified Date  :
  *************************************************/
...
--      v_errmsg := 'CAF has to be generated atleast once for this pan';
   ...
   BEGIN
      SELECT cfm_txn_code, cfm_txn_mode, cfm_delivery_channel, cfm_txn_type
        ...
        FROM cms_func_mast
       WHERE cfm_func_code = 'DHTLST' AND cfm_inst_code=prm_instcode;
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
