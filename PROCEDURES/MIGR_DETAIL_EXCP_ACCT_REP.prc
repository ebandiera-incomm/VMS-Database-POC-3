CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_DETAIL_EXCP_ACCT_REP 
AS
   ...
   IS
             ...
             SELECT mad_file_name FileName, 
               ...
               cae_acct_numb AccountNumber, 
               cae_eror_mesg ErrorMessage
          FROM VMSCMS.migr_acct_data_temp, VMSCMS.migr_cust_acct_err
         WHERE cae_acct_numb = mad_acct_numb
         AND   mad_proc_flag = 'N' 
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'DETAIL_EXCP_ACCT_REP.csv';
         ...
         THEN
            ...
            BEGIN
               INSERT INTO migr_rpt_wrt_fail
                           (mrw_file_name, mrw_errmsg,mrw_line_number, mrw_sqlerrmsg,MRW_RPT_FILE
                           )
                    VALUES (cur_data.filename,v_errmsg,cur_data.linenumber,v_sqlerrmsg,l_file_name
                           );
            EXCEPTION
               WHEN OTHERS
               THEN
