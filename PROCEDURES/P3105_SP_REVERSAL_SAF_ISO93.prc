CREATE OR REPLACE PROCEDURE VMSCMS.P3105_SP_REVERSAL_SAF_ISO93(P_INST_CODE           IN NUMBER,
...
BEGIN
  ...
  IF V_DELCHANNEL_CODE <> P_DELV_CHNL THEN
    ...
    IF (P_MSG_TYP NOT IN ('1220','1221')) OR
      (P_RVSL_CODE = '00') THEN
  ...
  --Added by srinivasu
   ...
   IF P_MSG_TYP = '1221' or P_MSG_TYP ='1220' THEN


    SELECT COUNT(*)
     INTO V_SAF_TXN_COUNT
     FROM TRANSACTIONLOG
    WHERE ORGNL_RRN = P_ORGNL_RRN  AND ORGNL_BUSINESS_DATE = P_ORGNL_BUSINESS_DATE
         AND CUSTOMER_CARD_NO = V_HASH_PAN --P_card_no
         AND INSTCODE = P_INST_CODE AND
         --ORGNL_TERMINAL_ID = P_ORGNL_TERMINAL_ID
           RESPONSE_CODE = '00' AND
         MSGTYPE in ('1220','1221');
       ...
       ELSE
        IF P_ORGNL_BUSINESS_DATE = P_BUSINESS_DATE THEN
          ...
          UPDATE CMS_TRANSLIMIT_CHECK
           ...
           IF SQL%ROWCOUNT = 0 THEN
               V_ERRMSG  := 'Problem while updating data in CMS_TRANSLIMIT_CHECK'||SUBSTR(SQLERRM, 1, 300);
               V_RESP_CDE := '21';
               RAISE EXP_RVSL_REJECT_RECORD;
          END IF;
    ...
    BEGIN
     ...
     SELECT CTC_ATMUSAGE_AMT,
       ...
       INTO V_ATM_USAGEAMNT,
           ...
           V_MMPOS_USAGEAMNT
       FROM CMS_TRANSLIMIT_CHECK
      WHERE  CTC_PAN_CODE = trim(V_HASH_PAN) and CTC_INST_CODE = P_INST_CODE
         AND CTC_MBR_NUMB = trim(P_MBR_NUMB);
    EXCEPTION
     WHEN NO_DATA_FOUND THEN
       V_ERRMSG   := 'Cannot get the Transaction Limit Details of the Card' ||
                  V_RESP_CDE || SUBSTR(SQLERRM, 1, 300);
