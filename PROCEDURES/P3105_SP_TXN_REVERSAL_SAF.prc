CREATE OR REPLACE PROCEDURE VMSCMS.P3105_SP_TXN_REVERSAL_SAF (P_INST_CODE           IN NUMBER,
...
BEGIN
  ...
  --Added by srinivasu
   ...
   IF P_MSG_TYP = '9221' or P_MSG_TYP ='9220' THEN
	...
	WHERE ORGNL_RRN = P_ORGNL_RRN  AND ORGNL_BUSINESS_DATE = P_ORGNL_BUSINESS_DATE
		 ...
		 AND INSTCODE = P_INST_CODE AND
		 ORGNL_TERMINAL_ID = P_ORGNL_TERMINAL_ID AND RESPONSE_CODE = '00' AND
  ...
  raise EXP_RVSL_REJECT_RECORD;
         when others then
          V_ERRMSG := 'Error while updating the card status'||substr(sqlerrm,1,200);
                    V_RESP_CDE := '21';
                    RAISE EXP_RVSL_REJECT_RECORD;
  end;
  END IF;
