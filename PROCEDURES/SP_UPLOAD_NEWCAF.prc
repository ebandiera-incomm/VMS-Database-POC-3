CREATE OR REPLACE PROCEDURE VMSCMS.sp_upload_newcaf(
   prm_instcode   IN       NUMBER,
   prm_filename   IN       VARCHAR2,
   prm_filerectype    IN   VARCHAR2,
...
AS
   ...
   *************************************************/
   ...
   -- v_func_desc             cms_func_mast.cfm_func_desc%TYPE; -- commented for fwr-48
   -- v_spprt_funccode        cms_func_mast.cfm_func_code%TYPE; -- commented for fwr-48
   -- v_spprtfunc_desc        cms_func_mast.cfm_func_desc%TYPE; -- commented for fwr-48
   -- v_check_spprtfunccode   NUMBER (1); -- commented for fwr-48
   -- v_check_funccode        NUMBER (1); -- commented for fwr-48
   ...
   v_comm_addr_lin1        cms_caf_info_temp.cci_seg12_addr_line1%TYPE;
   v_comm_addr_lin2        cms_caf_info_temp.cci_seg12_addr_line2%TYPE;
   v_comm_postal_code      cms_caf_info_temp.cci_seg12_postal_code%TYPE;
   v_comm_homephone_no     cms_caf_info_temp.cci_seg12_homephone_no%TYPE;
   v_comm_mobileno         cms_caf_info_temp.cci_seg12_mobileno%TYPE;
   v_comm_emailid          cms_caf_info_temp.cci_seg12_emailid%TYPE;
   v_comm_city             cms_caf_info_temp.cci_seg12_city%TYPE;
   v_comm_state            cms_caf_info_temp.cci_seg12_state%TYPE;
   ...
   v_other_addr_lin1       cms_caf_info_temp.cci_seg13_addr_line1%TYPE;
   v_other_addr_lin2       cms_caf_info_temp.cci_seg13_addr_line2%TYPE;
   v_other_postal_code     cms_caf_info_temp.cci_seg13_postal_code%TYPE;
   v_other_homephone_no    cms_caf_info_temp.cci_seg13_homephone_no%TYPE;
   v_other_mobileno        cms_caf_info_temp.cci_seg13_mobileno%TYPE;
   v_other_emailid         cms_caf_info_temp.cci_seg13_emailid%TYPE;
   v_other_city            cms_caf_info_temp.cci_seg13_city%TYPE;
   v_other_state           cms_caf_info_temp.cci_seg12_state%TYPE;
   ...
   v_comm_officeno         cms_caf_info_entry.cci_seg12_officephone_no%type;  --added on 15th oct'10
   v_other_officeno         cms_caf_info_entry.cci_seg13_officephone_no%type; --added on 15th oct'10
   ...
   IS
      SELECT   TRIM (cci_inst_code) cci_inst_code,
               ...
               TRIM (cci_hobbies) cci_hobbies, TRIM (cci_cust_id)
                                                                 cci_cust_id,
           TRIM(CCI_CUST_CATG) cci_cust_catg,
           ...
           TRIM (cci_payment_mode) cci_payment_mode,
              ...
              -- TRIM (cci_exchange_rate) cci_exchange_rate,
               ...
               TRIM (cci_corp_id) cci_corp_id,TRIM(cci_merc_code) cci_merc_code,
               trim(cci_ikit_flag) cci_ikit_flag,
...
BEGIN                                                        --<< MAIN BEGIN>>
   ...
   LOOP                                            --<< LOOP C1(FILE WISE) >>
      ...
      --Sn check filename for the instcode
     BEGIN
             SELECT 1
           INTO   v_check_file
             FROM      cms_upload_ctrl
           WHERE  cuc_inst_code = prm_instcode
           AND    cuc_file_name = prm_filename;
     
      EXCEPTION
    
             WHEN NO_DATA_FOUND THEN
           v_errmsg :=
                  'Invalid file for institute or file not present';
            RAISE exp_reject_file;
         WHEN OTHERS
         THEN
            v_errmsg :=
                  'Error while selecting file detail for institute '
               || SUBSTR (SQLERRM, 1, 200);
            RAISE exp_reject_file;
      
      END;
      --En check filename for the instcode
      ...
      BEGIN
         INSERT INTO cms_upload_summary
              VALUES (prm_instcode, i1.cuc_file_name, 0, 0, 0, prm_lupduser,
                      SYSDATE, prm_lupduser, SYSDATE,prm_filerectype);
      EXCEPTION
      ...
      BEGIN                                    --<< BEGIN LOOP C1(FILE WISE)>>
         ...
         LOOP                          --<< LOOP C2(FILE WISE RECORD WISE) >>
            ...
            BEGIN               --<< BEGIN LOOP C2 (FILE WISE RECORD WISE) >>
               ...
               EXCEPTION
                  ...
                  THEN
                    ...
                    BEGIN
                     ...
                     IF v_errmsg<>'OK'
                     THEN
                          v_errmsg:='Error from set branch '||substr(sqlerrm,1,200);
                         RAISE exp_reject_record;
                     END IF;
                                                  
                    EXCEPTION                      
                      WHEN OTHERS THEN
                             v_errmsg := 'Error while creating cust catg ' || substr(sqlerrm,1,200);
                             RAISE exp_reject_record;            
                     END;
               ...
               --Sn check cntry mast
                ...
                EXCEPTION
                   WHEN NO_DATA_FOUND
                   THEN
                      --v_errmsg :='Country is not defined in master'|| i2.cci_seg12_country_code;        
                      --RAISE exp_reject_record;
                      sp_create_country(
                                        prm_instcode,
                                        prm_filename,
               ...
               BEGIN
                  SELECT ccc_catg_code, ccc_catg_sname
                    ...
                    FROM cms_cust_catg
                   WHERE ccc_catg_sname = i2.cci_cust_catg
                   AND      ccc_inst_code  = prm_instcode;
               EXCEPTION
                  WHEN NO_DATA_FOUND
                  THEN
                        BEGIN
               ...
               BEGIN
                    ...
                    FROM   CMS_BIN_MAST
                    WHERE  cbm_inst_bin = i2.cci_pan_code
                    AND    cbm_inst_code = prm_instcode;
                       
                    IF v_check_bin_stat <> '1'  THEN
                       v_errmsg := 'Not a active Bin ' || i2.cci_pan_code;
                       RAISE exp_reject_record;
                    END IF;
               
               EXCEPTION
                      WHEN NO_DATA_FOUND THEN
                   v_errmsg := 'Bin ' || i2.cci_pan_code ||' not found in master ';
                   RAISE exp_reject_record;
                   ...
                   WHEN OTHERS THEN
                   v_errmsg := 'Error while selecting bin details from master '|| substr(sqlerrm,1,200) ;
                   RAISE exp_reject_record;
            ...
            --Sn check prod bin
               ...
               --Sn check card type
               BEGIN
                  SELECT cpc_card_type, cpc_encrypt_enable
                    INTO v_card_type, v_encrypt_enable
               ...
               EXCEPTION
                  ...
                  THEN
                     ...
                     EXCEPTION
                        ...
                        THEN
                           v_errmsg :=
                                   'Error while creating a product and customer category relation ' || substr(sqlerrm,1,150);
                           RAISE exp_reject_record;
                     END;
                  WHEN OTHERS
                  THEN
                  ...
                  BEGIN
                     SELECT 1
                      ...
                      WHERE cfp_prod_code = v_prodcode
                        ...
                        AND cfp_inst_code = prm_instcode;
                  EXCEPTION
                     WHEN NO_DATA_FOUND
                     THEN
                        v_errmsg :=
			...
			IF v_encrypt_enable = 'Y' THEN
			   v_encr_firstname := fn_dmaps_main(i2.cci_seg12_name_line1);     
			   v_encr_lastname := fn_dmaps_main(i2.cci_seg12_name_line2);
			   v_encr_mothers_name := fn_dmaps_main(i2.cci_mother_name);
			ELSE
			   v_encr_firstname := i2.cci_seg12_name_line1;     
			   v_encr_lastname := i2.cci_seg12_name_line2;
			   v_encr_mothers_name := i2.cci_mother_name;
			END IF;
               ...
               EXCEPTION
                ...
                SELECT DECODE (UPPER (i2.cci_title),
                 ...
                 --Sn assign records to customer gen variable
                    BEGIN
                        SELECT      type_cust_rec_array
                                            (i2.cci_customer_param1, 
                                            i2.cci_customer_param2, 
                                            i2.cci_customer_param3, 
                                            i2.cci_customer_param4, 
                                            i2.cci_customer_param5, 
                                            i2.cci_customer_param6, 
                                            i2.cci_customer_param7, 
                                            i2.cci_customer_param8, 
                                            i2.cci_customer_param9, 
                                            i2.cci_customer_param10 
                                            )
                          INTO        
                                    v_cust_data
                         from dual;
...
---   ************************************************************************************         
 ...
 BEGIN
            --Sn create communication address
                                         SELECT DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_addr_line1),i2.cci_seg12_addr_line1),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_addr_line1),i2.cci_seg13_addr_line1)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_addr_line2),i2.cci_seg12_addr_line2),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_addr_line2),i2.cci_seg13_addr_line2)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_postal_code),i2.cci_seg12_postal_code),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_homephone_no),i2.cci_seg13_homephone_no)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_homephone_no),i2.cci_seg12_homephone_no),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_homephone_no),i2.cci_seg13_homephone_no)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_mobileno),i2.cci_seg12_mobileno),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_mobileno),i2.cci_seg13_mobileno)
                                                       ),
                                                 DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_officephone_no,
                                                         i2.cci_seg13_officephone_no
                                                        ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_emailid),i2.cci_seg12_emailid),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_emailid),i2.cci_seg13_emailid)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_city),i2.cci_seg12_city),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_city),i2.cci_seg13_city)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_state,
                                                        i2.cci_seg13_state
                                                ...
                                                DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_country_code,
                                                             i2.cci_seg13_country_code
                                                       ), 
                                --Sn assign other gen comm adddress
                                 type_addr_rec_array
                                 (
                                    DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param1,
                                                        i2.cci_seg13_addr_param1
                                                       ),
                                    DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param2,
                                                        i2.cci_seg13_addr_param2
                                                       ),
                                    DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param3,
                                                        i2.cci_seg13_addr_param3
                                                       ),
                                    DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param4,
                                                        i2.cci_seg13_addr_param4
                                                       ),
                                    DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param5,
                                                        i2.cci_seg13_addr_param5
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param6,
                                                        i2.cci_seg13_addr_param6
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param7,
                                                        i2.cci_seg13_addr_param7
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param8,
                                                        i2.cci_seg13_addr_param8
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param9,
                                                        i2.cci_seg13_addr_param9
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg12_addr_param10,
                                                        i2.cci_seg13_addr_param10
                                                       )), 
                                                --En assign other gen comm address
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_addr_line1),i2.cci_seg13_addr_line1),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_addr_line1),i2.cci_seg12_addr_line1)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_addr_line2),i2.cci_seg13_addr_line2),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_addr_line2),i2.cci_seg12_addr_line2)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_postal_code),i2.cci_seg13_postal_code),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_postal_code),i2.cci_seg12_postal_code)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_homephone_no),i2.cci_seg13_homephone_no),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_homephone_no),i2.cci_seg12_homephone_no)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_mobileno),i2.cci_seg13_mobileno),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_mobileno),i2.cci_seg12_mobileno)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_officephone_no,
                                                         i2.cci_seg12_officephone_no
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_emailid),i2.cci_seg13_emailid),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_emailid),i2.cci_seg12_emailid)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg13_city),i2.cci_seg13_city),
                                                        decode(V_ENCRYPT_ENABLE,'Y', fn_dmaps_main(i2.cci_seg12_city),i2.cci_seg12_city)
                                                       ),
                                                DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_state,
                                                        i2.cci_seg12_state
                                                ...
                                                (
                                                 DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param1,
                                                        i2.cci_seg12_addr_param1
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param2,
                                                        i2.cci_seg12_addr_param2
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param3,
                                                        i2.cci_seg12_addr_param3
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param4,
                                                        i2.cci_seg12_addr_param4
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param5,
                                                        i2.cci_seg12_addr_param5
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param6,
                                                        i2.cci_seg12_addr_param6
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param7,
                                                        i2.cci_seg12_addr_param7
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param8,
                                                        i2.cci_seg12_addr_param8
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param9,
                                                        i2.cci_seg12_addr_param9
                                                       ),
                                                  DECODE (i2.cci_comm_type,
                                                        '0', i2.cci_seg13_addr_param10,
                                                        i2.cci_seg12_addr_param10
                                                       )
                                                       )
                                                --En assign other gen comm address
                                           INTO v_comm_addr_lin1,
                                         ...
                                         --Sn create communication address
                                         IF v_encrypt_enable = 'Y' THEN
										    v_encr_p_add_one := fn_dmaps_main(i2.cci_seg12_addr_line1);
											v_encr_m_add_one := fn_dmaps_main(i2.cci_seg13_addr_line1);
										 ELSE
										    v_encr_p_add_one := i2.cci_seg12_addr_line1;
											v_encr_m_add_one := i2.cci_seg13_addr_line1;
										 END IF;
										 ...
										 IF v_comm_addr_lin1 IS NOT NULL
                                         THEN
                                          IF v_comm_addr_lin1 = v_encr_p_add_one 
                                          THEN
                                                 v_comm_type := 'R';
                                          ELSIF v_comm_addr_lin1 = v_encr_m_add_one
                                          THEN
                                                  v_comm_type := 'O';
                                          END IF;
                                            BEGIN
                                               SELECT gcm_cntry_code
                                                 INTO v_gcm_cntry_code
                                                 FROM gen_cntry_mast
                                                WHERE gcm_curr_code = v_comm_cntrycode
                                                AND   GCM_INST_CODE = prm_instcode;
                                               sp_create_addr (prm_instcode,
                                                               v_cust_code,
                                                               v_comm_addr_lin1,
                              ...
                              END IF;
                                            ...
                                            BEGIN
                                               SELECT gcm_cntry_code
                                                 ...
                                                 FROM gen_cntry_mast
                                                 WHERE gcm_curr_code = v_other_cntrycode
                                                 AND   gcm_inst_code = prm_instcode;
                                               sp_create_addr (prm_instcode,
                                                               v_cust_code,
                                                               v_other_addr_lin1,
        ...
        raise;
        WHEN others THEN
        v_errmsg := 'Error while creating address ' || substr(sqlerrm,1,200);
        RAISE exp_reject_record;
        END;
        --En create address
---   ************************************************************************************         
               ...
               BEGIN
                  SELECT cat_type_code
                   ...
                   WHERE cat_inst_code = prm_instcode
                     AND cat_switch_type = i2.cci_seg31_typ;
               EXCEPTION
                  WHEN NO_DATA_FOUND
                  THEN
                     v_errmsg :=
                          'Acct type not defined for  ' || i2.cci_seg31_typ;
                     RAISE exp_reject_record;
                  WHEN OTHERS
                  THEN
                     v_errmsg :=
               ...
               BEGIN
                  SELECT cas_stat_code
                   ...
                   WHERE cas_inst_code = prm_instcode
                     AND cas_switch_statcode = i2.cci_seg31_stat;
               EXCEPTION
                  WHEN NO_DATA_FOUND
                  THEN
                     v_errmsg :=
                          'Acct stat not defined for  ' || i2.cci_seg31_stat;
                     RAISE exp_reject_record;
                  WHEN OTHERS
                  THEN
                     v_errmsg :=
               ...
               --Sn select gen acct data 
                   BEGIN
                    SELECT
                        type_acct_rec_array
                        (
                        i2.cci_seg31_num_param1,    
                        i2.cci_seg31_num_param2,    
                        i2.cci_seg31_num_param3,    
                        i2.cci_seg31_num_param4,    
                        i2.cci_seg31_num_param5,    
                        i2.cci_seg31_num_param6,    
                        i2.cci_seg31_num_param7,    
                        i2.cci_seg31_num_param8,    
                        i2.cci_seg31_num_param9,    
                        i2.cci_seg31_num_param10
                        )
                    INTO
                        v_seg31acctnum_data
                     FROM DUAL;
                EXCEPTION
                         WHEN OTHERS THEN
                         v_errmsg :=
                           'Error while selecting acct data '
                           || SUBSTR (SQLERRM, 1, 200);
                            RAISE exp_reject_record;
               END;
...
--added on 11/10/2002 ...gets the card validity period in months from parameter table
               ...
               --Sn select gen acct data 
                   BEGIN
                    SELECT
                        type_appl_rec_array
                        (
                        i2.cci_custappl_param1,    
                        i2.cci_custappl_param2,    
                        i2.cci_custappl_param3,    
                        i2.cci_custappl_param4,    
                        i2.cci_custappl_param5,    
                        i2.cci_custappl_param6,    
                        i2.cci_custappl_param7,    
                        i2.cci_custappl_param8,    
                        i2.cci_custappl_param9,    
                        i2.cci_custappl_param10)
                    INTO
                        v_appl_data
                     FROM DUAL;
                EXCEPTION
                         WHEN OTHERS THEN
                         v_errmsg :=
                           'Error while selecting appl data '
                           || SUBSTR (SQLERRM, 1, 200);
                            RAISE exp_reject_record;
               END;
			   ...
			   IF v_encrypt_enable = 'Y' THEN
				  v_encr_lastname := SUBSTR (fn_dmaps_main(i2.cci_seg12_name_line2), 1, 30);
			   ELSE
				  v_encr_lastname := SUBSTR (i2.cci_seg12_name_line2, 1, 30);
			   END IF;
