CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_DETAIL_EXCP_CUST_LOAD_REP 
AS
   ...
   IS
    SELECT MCE_FILE_NAME FileName,
           MCE_RECORD_NUMBER LineNumber,
           VMSCMS.FN_MASK(MCE_CARD_NUMBER,'X',7,6) CardNumber,
           MCE_PROCESS_MSG ErrorMessage
    FROM VMSCMS.MIGR_CUST_DATA_EXCP
	where MCE_FILE_NAME = prm_file_name;

   CURSOR cur_file 
   IS
      SELECT  MCE_FILE_NAME filename
                 FROM vmscms.MIGR_CUST_DATA_EXCP
                group by MCE_FILE_NAME
             ORDER BY MCE_FILE_NAME ;
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'DETAIL_EXCP_CUST_LOAD_REP.csv';
