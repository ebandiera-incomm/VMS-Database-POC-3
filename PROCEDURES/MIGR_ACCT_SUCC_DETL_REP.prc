CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_ACCT_SUCC_DETL_REP 
AS
   ...
   IS
    SELECT mad_file_name FileName, 
           mad_record_numb  LineNumber, 
           mad_acct_numb AccountNumber, 
           mad_acct_stat AccountStatus,
           mad_avail_bal AccountBalance, 
           mad_ledg_bal LedgerBalance
      FROM VMSCMS.migr_acct_data_temp
     WHERE mad_proc_flag in('N','P') 
	 and  mad_file_name = prm_file_name
	 AND mad_acct_numb NOT IN (SELECT nvl(cae_acct_numb,0) FROM VMSCMS.migr_cust_acct_err)
     order by mad_file_name,mad_record_numb  ;

   CURSOR cur_file 
   IS
      SELECT  MAD_FILE_NAME filename
                 FROM vmscms.migr_acct_data_temp
                group by MAD_FILE_NAME
             ORDER BY MAD_FILE_NAME ;
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'ACCT_SUCC_DETL_REP.csv';
   ...
   THEN
      ...
      UTL_FILE.put (l_file, 'ACCOUNT NUMBER,');
      UTL_FILE.put (l_file, 'ACCOUNT STATUS,');
      UTL_FILE.put (l_file, 'ACCOUNT BALANCE,');
      UTL_FILE.put (l_file, 'LEDGER BALANCE,');
      ...
      LOOP
         ...
         THEN
            BEGIN
               ...
               UTL_FILE.put (l_file, cur_data.AccountNumber || ',');
               UTL_FILE.put (l_file, cur_data.AccountStatus || ',');
               UTL_FILE.put (l_file, cur_data.AccountBalance || ',');
               UTL_FILE.put (l_file, cur_data.LedgerBalance || ',');
