CREATE OR REPLACE PROCEDURE VMSCMS.sp_dup_acct_activcrd (
   ...
   BEGIN
      INSERT INTO cms_dup_acct_pan
                  (cda_acct_no,cda_acct_id, cda_pan_code,cda_new_acct_no,
                   cda_new_acct_id, cda_cust_code, cda_card_stat,cda_mask_pan,
                   cda_process_flag, cda_process_msg, cda_ins_date
                  )
           VALUES (prm_acct_no,prm_acct_id, prm_pan_code,prm_new_acct_no,
                   prm_new_acct_id,prm_cust_code, prm_card_stat,prm_mask_pan,
                   prm_process_flag, prm_process_msg, SYSDATE
                  );
   ...
   LOOP
     ...
     BEGIN
      FOR z IN (SELECT   cap_pan_code, cap_prod_code, cap_card_type,
                         cap_appl_bran, cap_cust_code, cap_bill_addr,cap_mask_pan,
                         cap_card_stat, cap_acct_id, cap_acct_no,
                         ROWID row_id
                    FROM cms_appl_pan
                   WHERE cap_inst_code = 1 AND cap_acct_no = x.cap_acct_no AND CAP_STARTERCARD_FLAG='Y'
                ORDER BY cap_card_stat DESC)
      LOOP
         BEGIN
            IF z.cap_card_stat <> 1
            THEN
      ...
      IF V_ERRMSG='OK'
          THEN
            ...
            WHEN OTHERS
            THEN
              V_ERRMSG:='Error while updating success flag-'||SUBSTR(SQLERRM,1,200);
