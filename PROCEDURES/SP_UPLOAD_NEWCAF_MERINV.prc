create or replace PROCEDURE        vmscms.SP_UPLOAD_NEWCAF_MERINV (
...
AS
   /*************************************************
       * Created Date     :  12-June-2012
       * Created By       :  Dhiraj Gaikwad
       * PURPOSE          : Generating application and card for inventory
       * Modified By      : Amit Sonar
       * Modified Date    : 14-Sep-2012
       * Modified Reason  : Process flag updation after serial PAN generation
       * Reviewer         : B.Besky Anand
       * Reviewed Date    : 14-SEP-2012
       * Release Number   : CMS3.5.1_RI00016_B0008
       ...
       * Modified By      : Ramesh
       * Modified Date    : 18-MAY-2014
       * Modified Reason  : FSS-1648 :
       * Reviewer         :
       * Reviewed Date    :
       * Release Number   : RI0027.1.5.1
     ...
     * Modified by      : Siva Kumar M
     * Modified For     : Mantis id:16413
     * Modified Date    : 02-June-2016
     * Modified reason  : Changes in Inventory Card Generation
     * Reviewer         : Saravankumar/Spankaj
     * Build Number     : VMSGPRHOST4.1_B0003
   ...
   *************************************************/
    ...
    V_CNTRY_CODE           GEN_CNTRY_MAST.GCM_CNTRY_CODE%TYPE; --Added for MVHOST-866
   ...
   v_encr_dummy                 CMS_CAF_INFO_ENTRY.CCI_SEG12_NAME_LINE1%TYPE;
...
--Added for MVHOST-866
  ...
  EXCEPTION
    WHEN OTHERS THEN
     p_errmsg := 'Error while selecting gen_cntry_mast ' ||
                SUBSTR(SQLERRM, 1, 200);
     RAISE exp_reject_file;
  END;
--End  --Added for MVHOST-866
   ...
   FOR i1 IN (SELECT ordr.cmo_merprodcat_id,prodcat.cmp_inst_code,
                     ...
                     ordr.cmo_nocards_ordr,ordr.cmo_ins_user,cattype.cpc_serl_flag,cattype.CPC_STARTERCARD_DISPNAME,
					 cattype.cpc_profile_code,cattype.cpc_encrypt_enable,cattype.CPC_PAN_INVENTORY_FLAG,cmo_raise_flag
                FROM cms_merinv_prodcat prodcat, cms_merinv_ordr ordr, cms_prod_cattype cattype
               WHERE cmp_merprodcat_id = ordr.cmo_merprodcat_id
 ...
 If(I1.Cpc_Pan_Inventory_Flag <> 'Y')
Then
Begin
         SELECT COUNT (*)
           INTO v_merpan_cnt
         ...
         THEN
           ...
           /* IF i1.cpc_serl_flag = 1
            THEN
               SELECT COUNT (*)
                 INTO v_shuffleno_cnt
                 FROM cms_shfl_serl
                WHERE css_inst_code = i1.cmp_inst_code
                  AND css_prod_code = i1.cmp_prod_code
                  AND css_prod_catg = i1.cmp_prod_cattype
                  AND css_serl_flag = 0;
               ...
               THEN
                  ...
                  EXCEPTION
                     ...
                     THEN
                        ...
                        RAISE exp_reject_file;
                  END;
               ELSIF v_shuffleno_cnt >= i1.cmo_nocards_ordr
               THEN
                  v_order_process_cnt := i1.cmo_nocards_ordr;
               ELSIF v_shuffleno_cnt < i1.cmo_nocards_ordr
               THEN
                  -- v_order_process_cnt := v_shuffleno_cnt;
                  p_errmsg := 'Available shuffle count is less than the order number.';
                  ...
                  BEGIN
                   SELECT GCM_CNTRY_CODE
                    ...
                    WHERE GCM_CURR_CODE = (select cbp_param_value
                       ...
                       and CBP_INST_CODE = P_INSTCODE AND
                           CBP_PROFILE_CODE = i1.cpc_profile_code) AND 
                           GCM_INST_CODE = p_instcode;
                  EXCEPTION
                     WHEN OTHERS THEN
                    p_errmsg := 'Error while selecting gen_cntry_mast ' ||
               ...
               LOOP
                  BEGIN
                     IF i1.cpc_encrypt_enable = 'Y' THEN
					    v_encr_firstname := fn_emaps_main(i1.CPC_STARTERCARD_DISPNAME);
						v_encr_dummy := fn_emaps_main('*');
					 ELSE
					    v_encr_firstname := i1.CPC_STARTERCARD_DISPNAME;
						v_encr_dummy := '*';
				     END IF;
					 ...
					 INSERT INTO cms_caf_info_inv
                          ...
                          VALUES (i1.cum_file_name, j, i1.cmp_inst_code,
                                  ...
                                  '*',
                                 -- 'Alwin',
                                 nvl(v_encr_firstname,'Alwin'),
                                  v_encr_dummy,
                                  v_encr_dummy,
                                  v_encr_dummy, v_encr_dummy,
                                  'GA', v_encr_dummy,          --Modified for MVHOST-866
                                  V_CNTRY_CODE, 11,  --Modified for MVHOST-866
                                  NULL, 3,
           ...
           -- END IF;
            ...
            BEGIN
               INSERT INTO cms_summary_merinv
                    VALUES (p_instcode, i1.cum_file_name, 0, 0, 0,
                            p_lupduser, SYSDATE, p_lupduser, SYSDATE, '');
            EXCEPTION
            ...
            LOOP
               ...
               BEGIN
                  ...
                  -----------------------------------------------
--SN : Selecting prod catg code from prod_mast
                  ...
                  END;
				    ...
				    BEGIN
                        SELECT prod.cpm_profile_code,prod.cpm_catg_code,
							   ...
							   cattype.cpc_profile_code,cattype.cpc_encrypt_enable
                          INTO v_profile_code, v_cpm_catg_code,
                               v_prod_prefix, v_programid,
                               v_starter_card,v_exp_date_exemption,
							   v_profile_code_catg, v_encrypt_enable
...
-------------------------------------------------
           ...
           v_encr_firstname := fn_dmaps_main(y.cci_seg12_name_line1);
					 v_encr_lastname := fn_dmaps_main(y.cci_seg12_name_line2);                 
					 v_encr_p_add_one := fn_dmaps_main(y.cci_seg12_addr_line1);          
					 v_encr_p_add_two := fn_dmaps_main(y.cci_seg12_addr_line2);         
					 v_encr_zipcode := fn_dmaps_main(y.cci_seg12_postal_code);  
					 v_encr_city := fn_dmaps_main(y.cci_seg12_city);  					 
				  ELSE
           v_encr_firstname:= y.cci_seg12_name_line1;
					 v_encr_lastname := y.cci_seg12_name_line2;                 
					 v_encr_p_add_one := y.cci_seg12_addr_line1;          
					 v_encr_p_add_two := y.cci_seg12_addr_line2;         
					 v_encr_zipcode := y.cci_seg12_postal_code; 
					 v_encr_city := y.cci_seg12_city;  					 
				  END IF;
...
-- product catg code is passed using varible value from cms_prod_mast --sagar - 06Apr2012
                    ...
                    BEGIN
                     ...
                     BEGIN
                        sp_create_addr (p_instcode,
                                        ...
                                        --V_GCM_CNTRY_CODE,
                                        V_CNTRY_CODE, --Modified for FSS-1648
                  ...
                  THEN
                     ...
                     BEGIN
                        SELECT cbm_interchange_code
                          ...
                          FROM cms_bin_mast
                         -- CMS_PRODTYPE_MAP this table is not in use
      ...
      END;
  Else
         vmscard_stock.Merinv_Process(P_Instcode,
                                      P_Lupduser,
                                      i1.cmo_raise_flag,
                                      p_errmsg
                                      );
