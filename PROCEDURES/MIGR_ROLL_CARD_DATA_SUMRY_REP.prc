CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_ROLL_CARD_DATA_SUMRY_REP 
AS
   ...
   IS
    ...
    where mci_proc_flag='S' and MCI_MIGR_SEQNO = PRM_SEQNO) RECORDS_MIGRATED,
    (select MRC_DEL_CNT from vmscms.MIGR_ROLL_COUNT 
     where MRC_TABLE_NAME = 'CMS_APPL_PAN' and MRC_MIGR_SEQNO =PRM_SEQNO
    ...
    (select MRC_DEL_CNT from vmscms.MIGR_ROLL_COUNT 
     ...
     (
      ...
      where MRC_TABLE_NAME = 'CMS_APPL_PAN' and MRC_MIGR_SEQNO =PRM_SEQNO) - 
      (select count(*) from VMSCMS.migr_caf_info_entry
      where mci_proc_flag='S' and MCI_MIGR_SEQNO = PRM_SEQNO
      ) 
     )
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'ROLL_CARD_DATA_SUMRY_REP.csv';
