CREATE OR REPLACE PROCEDURE VMSCMS.Sp_Hotlist_Pan_prepaid (
...
/*************************************************
     ...
     * PURPOSE          : Hot List card for a perticular card number
     * Modified By:    :
     * Modified Date  :
  *************************************************/
...
------------------------------------------------Sn check card number------------------------------------
   BEGIN
      SELECT cap_prod_catg, cap_card_stat, cap_cafgen_flag, cap_embos_flag, cap_pin_flag
        INTO v_cap_prod_catg, v_cap_card_stat, v_cap_cafgen_flag,v_cap_embos_flag,v_cap_pin_flag
        FROM CMS_APPL_PAN
...
------------------------- Sn For Debit Card, Authorization not Required-------------------------
IF v_cap_prod_catg = 'P' THEN 
   ...
   END IF;
END IF;
-------------------------En For Debit Card, Authorization not Required-------------------------
...
--------------------------------------------------EN update card stat-------------------------------------------
   ...
   BEGIN
      INSERT INTO CMS_PAN_SPPRT
           ...
           VALUES (prm_instcode, v_hash_pan-- prm_pancode
           , v_mbrnumb, v_cap_prod_catg,
                   'HTLST', prm_rsncode, prm_remark,
