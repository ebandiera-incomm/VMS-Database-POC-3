create or replace PROCEDURE        vmscms.SP_ENTRY_NEWCAF_PCMS(P_INSTCODE IN NUMBER,
  ...
  /*************************************************
      ...
      * Modified By      :  Narayana Swamy T
      * Modified Date    :  15-Mar-2012
      * Modified Reason  :  Last Name  not stored in CMS_CUST_MAST
     * Reviewer         :  Saravanakumar
     * Reviewed Date    :  06_Apr-2012
     * Build Number     :  RI0005_B0001
 ...
 -- V_INITIAL_SPPRTFLAG   VARCHAR2(1); --commented for fwr-48
  ...
  CURSOR C IS
    SELECT CCI_INST_CODE,
         ...
         nvl(fn_dmaps_main(CCI_SSN_encr),CCI_SSN) CCI_SSN,
         CCI_HOBBIES,
         CCI_CUST_ID,
         CCI_COMM_TYPE,
  ...
  FOR I IN C LOOP
    ...
    BEGIN
     ...
     BEGIN
       ...
       SELECT CPC_PROFILE_CODE,
        ...
        INTO V_PROFILE_CODE, V_CPM_CATG_CODE, V_PROD_PREFIX, V_PROGRAMID,v_exp_date_exemption, v_encrypt_enable
	...
	END IF;
     ...
     BEGIN
       ...
       --if cust found then populate the existing customer else generate new customer code and flow
       BEGIN
     ...
     IF V_CATG_CODE = 'P' THEN
       ...
       SP_ACCOUNT_CONSTRUCT(P_INSTCODE,
                        ...
                        I.CCI_PROD_CODE,
                        P_LUPDUSER,
                        I.CCI_CARD_TYPE, --added by ram.MK
                        T_ACCT_NUM,
                        V_ERRMSG);
       IF V_ERRMSG <> 'OK' THEN
