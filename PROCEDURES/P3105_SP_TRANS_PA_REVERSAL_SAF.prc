CREATE OR REPLACE PROCEDURE VMSCMS.P3105_SP_TRANS_PA_REVERSAL_SAF(P_INST_CODE           IN NUMBER,
...
BEGIN
  ...
  BEGIN
    ...
    TO_NUMBER(DECODE(CTM_TRAN_TYPE, 'N', '0', 'F', '1')),CTM_PREAUTH_FLAG
  ...
  IF P_MSG_TYP = '9221' THEN
    ...
    WHERE RRN = P_RRN AND BUSINESS_DATE = P_BUSINESS_DATE
         ...
         AND INSTCODE = P_INST_CODE AND
         TERMINAL_ID = P_ORGNL_TERMINAL_ID AND RESPONSE_CODE = '00' AND
         MSGTYPE = '9220';
  ...
  BEGIN
    ...
    IF V_UPD_HOLD_AMOUNT <= 0 THEN
     ...
     UPDATE CMS_PREAUTH_TRANSACTION
        SET CPT_TOTALHOLD_AMT     = V_UPD_HOLD_AMOUNT,
