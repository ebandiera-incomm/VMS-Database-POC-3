CREATE OR REPLACE PROCEDURE VMSCMS.SP_ATTACH_FEEPLAN_PRODCATG(P_INST_CODE         IN NUMBER,
 ...
 V_EX_FEEPLAN              CMS_PRODCATTYPE_FEES.CPF_FEE_PLAN%TYPE;  
 
 /*************************************************
      * Created By       :  Deepa
      * Created Date     :  10-Aug-2012
      * Purpose          :  To attach fee plan to Product Category
      * Modified By      :  
      * Modified Date    :  
      * Modified Reason  :  
...
ELSIF V_FEEPLAN_COUNT=1 THEN
     ...
     AND CPF_CARD_TYPE = P_CARD_TYPE AND CPF_INST_CODE =P_INST_CODE;
       EXCEPTION
     WHEN OTHERS THEN
     P_RESP_MSG := 'Error while Selecting the Existing FeePlan Date ' ||
                     SUBSTR(SQLERRM, 1, 200); 
           ...
           EXIT WHEN C_WAIVDEL_DET%NOTFOUND;
            ...
            DELETE FROM CMS_PRODCATTYPE_WAIV
                  WHERE CPW_INST_CODE = p_inst_code
                    ...
                    update CMS_PRODCATTYPE_WAIV_hist set CPW_CHNG_REASON='Fee Plan Date Modified'
                     WHERE CPW_INST_CODE = p_inst_code                    
                    AND CPW_PROD_CODE = P_PROD_CODE
                    AND CPW_CARD_TYPE = P_CARD_TYPE                   
                    AND CPW_FEE_PLAN = v_ex_feeplan                    
...
BEGIN           
            ...
            INSERT INTO CMS_PRODCATTYPE_FEES
            (CPF_INST_CODE, CPF_PROD_CODE, CPF_CARD_TYPE, CPF_VALID_FROM,
     ...
     VALUES (p_inst_code, P_PROD_CODE, P_CARD_TYPE, v_from_date,
             v_to_date, 'PC', p_ins_user, p_lupd_user,
