CREATE OR REPLACE PROCEDURE VMSCMS.sp_grp_card_renewal
  (
...
BEGIN --<< MAIN BEGIN >>
 ...
 if prm_branch_list is not null then
 v_bran_cnt := prm_branch_list.COUNT;
  ...
  if prm_bin_list is not null then
  ...
  ----------Sn start insta card check----------  /*added by amit on 24 Sep'10 for not to allow any supprt func on insta card.*/
   ...
   prm_errmsg:='Error while checking the instant card validation flag. '||substr(sqlerrm,1,200);
   return;
   END;
        ...
        BEGIN               --<<CARD WISE LOOP>>
         ...
         ------En to verify instant card --------------
          ...
          BEGIN
            ...
            AND trim(cci_pan_code)    =k.cap_pan_code
            AND cci_mbr_numb = k.cap_mbr_numb
            AND cci_file_gen = 'N'
          ...
          --En delete record from CAF
            ...
            sp_caf_rfrsh (prm_inst_code, Fn_Dmaps_Main(k.cap_pan_code_encr), '000', SYSDATE, 'C', NULL, 'RENEW', prm_lupd_user, Fn_Dmaps_Main(k.cap_pan_code_encr), v_errmsg );
           IF v_errmsg != 'OK' THEN
          ...
          BEGIN
                 ...
                 INTO v_inst_bin
                 from cms_prod_bin
                 where CPB_INST_CODE=prm_inst_code
                 AND CPB_PROD_CODE=k.cap_prod_code;
         ...
         BEGIN
            INSERT INTO cms_audit_log_process
                 ...
                 VALUES (prm_inst_code, '', '',
                         k.cap_pan_code, '', 'GROUP CARD RENEWAL',
                         'INSERT', 'SUCCESS', prm_ipaddr,
                         'CMS_PAN_SPPRT', '', k.cap_pan_code_encr,
                         prm_lupd_user, SYSDATE
                        );
         EXCEPTION
            --excp of begin 3
            WHEN OTHERS
      ...
      --end for failure status record
        ...
        WHEN exp_reject_record THEN
          ...
          sp_cardrenewal_errlog(
                                  prm_inst_code,
                                  Fn_DMaps_Main(k.cap_pan_code_encr),
                                  'Renew',
                                  prm_remark,
                                  k.cap_disp_name,
                                  k.cap_acct_no,
