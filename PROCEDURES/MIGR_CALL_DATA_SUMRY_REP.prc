CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_CALL_DATA_SUMRY_REP 
AS
   ...
   IS
    ...
    (select count(*) from VMSCMS.MIGR_CSR_CALLLOG_TEMP where MCC_FILE_NAME = prm_file_name) Total_Number_of_Records ,
    (select count(*) from VMSCMS.MIGR_CSR_CALLLOG_TEMP
    where MCC_proc_flag='S' and MCC_FILE_NAME = prm_file_name) Records_Migrated,
    (select count(*) from VMSCMS.MIGR_CSR_CALLLOG_TEMP
    where MCC_proc_flag='F' and MCC_FILE_NAME = prm_file_name) Records_Rejected
    From dual;

   CURSOR cur_file 
   IS
      SELECT  MCC_FILE_NAME filename
                 FROM vmscms.MIGR_CSR_CALLLOG_TEMP
                group by MCC_FILE_NAME
             ORDER BY MCC_FILE_NAME ;
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'CALL_DATA_SUMRY_REP.csv';
