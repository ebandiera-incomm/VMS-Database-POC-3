CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_DETAIL_EXCP_CALL_LOAD_REP 
AS
   ...
   IS
    SELECT MCD_FILE_NAME FileName,
           MCD_RECORD_NUMBER LineNumber,
           VMSCMS.FN_MASK(MCD_CARD_NUMBER,'X',7,6) CardNumber,
           MCD_PROCESS_MSG ErrorMessage
    FROM VMSCMS.MIGR_CALLOG_DATA_EXCP
	where MCD_FILE_NAME = prm_file_name;

   CURSOR cur_file 
   IS
      SELECT  MCD_FILE_NAME filename
                 FROM vmscms.MIGR_CALLOG_DATA_EXCP
                group by MCD_FILE_NAME
             ORDER BY MCD_FILE_NAME ;
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'DETAIL_EXCP_CALL_LOAD_REP.csv';
