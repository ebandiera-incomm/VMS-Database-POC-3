create or replace PROCEDURE        vmscms.SP_ACCOUNT_CONSTRUCT (
...
AS
     ...
     * Modified Date    :  26-Nov-2012
     * Modified Reason  :  Commented exception part, since it is already handled.
     * Reviewer         :
     * Reviewed Date    :
     ...
     * Modified Reason  :  Pragma autonomous added to avoid duplicate accouts
     * Modified for     :  MVHOST-264
     * Reviewer         :  Dhiarj
     * Reviewed Date    :  28-Mar-2013
     * Release Number   :  CMS3.5.1_RI0024_B0012
 ...
 *********************************************************************************/
   ...
   --resource_busy exception; -- SN: 20121116                    -- Commented on 28-Mar-2013
   --pragma exception_init (resource_busy,-54); -- SN: 20121116  -- Commented on 28-Mar-2013
   ...
   IS
      ...
      resource_busy     EXCEPTION; -- Added on 28-Mar-2013  to avoid duplicate account number
      PRAGMA EXCEPTION_INIT (resource_busy, -54); -- Added on 28-Mar-2013  to avoid duplicate account number
      PRAGMA AUTONOMOUS_TRANSACTION; -- Added on 28-Mar-2013 to avoid duplicate account number
   ...
   BEGIN
     ...
     -- BEGIN--Commented by Saravanakumar on 26-Nov-2012 since exception is handled below already
      ...
      SELECT cac_ctrl_numb,
       ...
       for update wait 1;-- Added on 28-Mar-2013  to avoid duplicate account number
       --for update nowait;--Added by Saravanakumar on 16-Nov-2012 to avoid duplicate account number


    /*EXCEPTION --Commented by Saravanakumar on 26-Nov-2012 since exception is handled below already
    ...
    --   IF v_ctrlnumb > v_max_serial_no
      ...
      THEN
         ...
         ROLLBACK; -- Added on 28-Mar-2013 to avoid duplicate account number
         RETURN;   -- Added on 28-Mar-2013 to avoid duplicate account number
      END IF;

      COMMIT; -- Added on 28-Mar-2013 to avoid duplicate account number
   ...
   EXCEPTION
      ...
      THEN
        ...
        WHEN resource_busy then --Added by Saravanakumar on 26-Nov-2012 for handling resource busy exception
...
BEGIN
   ...
   BEGIN
      SELECT cpc_profile_code,CPC_ACCT_PROD_PREFIX
       ...
       WHERE cpc_inst_code = prm_instcode AND cpc_prod_code = prm_prod_code and cpc_card_type=PROD_CATTYPE;
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
 ...
 *          ************************************************/
  ...
  /* FOR i IN 1 .. v_loop_max_cnt
   LOOP
      ...
      DBMS_OUTPUT.put_line (   'PRINT START FROM  I  '|| v_table_acct_construct (i).cac_start_from );
      DBMS_OUTPUT.put_line (   'PRINT FIELD NAME I  '|| v_table_acct_construct (i).cac_field_name );
      DBMS_OUTPUT.put_line (   'PRINT FIELD VALUE I  '|| v_table_acct_construct (i).cac_field_value );
      DBMS_OUTPUT.put_line (   'PRINT FIELD LENGTH I  ' || v_table_acct_construct (i).cac_length);
   END LOOP;
   */
