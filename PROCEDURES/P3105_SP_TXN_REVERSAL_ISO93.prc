CREATE OR REPLACE PROCEDURE VMSCMS.P3105_SP_TXN_REVERSAL_ISO93(P_INST_CODE           IN NUMBER,
...
BEGIN
  ...
  EXCEPTION
    WHEN OTHERS THEN
     V_ERRMSG := 'Error while converting pan hash '|| P_CARD_NO || SUBSTR(SQLERRM, 1, 200);
  ...
  EXCEPTION
    WHEN OTHERS THEN
     V_ERRMSG := 'Error while converting pan encr ' || P_CARD_NO || SUBSTR(SQLERRM, 1, 200);
     RAISE EXP_RVSL_REJECT_RECORD;
  END;
  --EN create encr pan
  ...
  BEGIN
    ...
    SELECT COUNT(1)
     INTO V_RRN_COUNT
     FROM TRANSACTIONLOG
    WHERE RRN = P_RRN AND BUSINESS_DATE = P_BUSINESS_DATE
  ...
  IF (V_DELCHANNEL_CODE <> P_DELV_CHNL) THEN
    ...
    IF (P_MSG_TYP NOT IN ('1420', '1421') ) OR
      (P_RVSL_CODE = '00') THEN
     V_RESP_CDE := '12';
  ...
  BEGIN
    SELECT CAM_ACCT_NO,CAM_ACCT_BAL, CAM_LEDGER_BAL
            INTO V_CARD_ACCT_NO,V_ACCT_BALANCE, V_LEDGE_BALANCE
     FROM CMS_ACCT_MAST
    WHERE CAM_ACCT_NO =
         (SELECT CAP_ACCT_NO
            FROM CMS_APPL_PAN
           WHERE CAP_PAN_CODE = V_HASH_PAN
                AND CAP_MBR_NUMB = V_MEMBER_NUMBER AND
 ...
 --Sn Check for maximum card balance configured for the product profile.
  ...
  SELECT  CFP_DRACCT_NO
      ...
      WHERE CFP_FUNC_CODE = V_FUNC_CODE AND
           ...
           IF V_ORGNL_DRACCT_NO IS NULL THEN
               ...
               IF ((V_ACCT_BALANCE + (V_REVERSAL_AMT+V_ORGNL_TXN_TOTALFEE_AMT)) > V_MAX_CARD_BAL) OR
                    ((V_LEDGE_BALANCE + (V_REVERSAL_AMT+V_ORGNL_TXN_TOTALFEE_AMT)) > V_MAX_CARD_BAL) THEN
  ...
  SELECT csl_trans_narrration CSL_TRANS_NARRRATION,CSL_MERCHANT_NAME,CSL_MERCHANT_CITY,CSL_MERCHANT_STATE
	 INTO V_TXN_NARRATION,V_TXN_MERCHNAME,V_TXN_MERCHCITY,V_TXN_MERCHSTATE--Mofified by Deepa on 09-May-2012 to include Merchant name,city and state in statements log
...
--En find narration
  ...
  BEGIN
    ...
    BEGIN
     ...
     SELECT CTC_ATMUSAGE_AMT,
      ...
      WHERE CTC_INST_CODE = P_INST_CODE AND CTC_PAN_CODE = V_HASH_PAN
           AND CTC_MBR_NUMB = V_MEMBER_NUMBER;
    EXCEPTION
     WHEN NO_DATA_FOUND THEN
    ...
    EXCEPTION
     WHEN OTHERS THEN
       V_ERRMSG   := 'Error while updating 51 CMS_TRANSLIMIT_CHECK ' ||SUBSTR(SQLERRM, 1, 200);
       V_RESP_CDE := '21';
       RAISE EXP_RVSL_REJECT_RECORD;
    END;
       ELSE
...
EXCEPTION
  ...
  WHEN EXP_RVSL_REJECT_RECORD THEN
   ...
   BEGIN
    ...
    EXCEPTION
     WHEN NO_DATA_FOUND THEN
       V_ERRMSG   := 'Cannot get the Transaction Limit Details of the Card--' ||V_HASH_PAN||P_INST_CODE||V_MEMBER_NUMBER;
       V_RESP_CDE := '21';
       RAISE EXP_RVSL_REJECT_RECORD;
       when others then
        V_ERRMSG   := 'Checking ' ||
    ...
    BEGIN
     ...
     VALUES
       (P_MSG_TYP,
        ...
        NULL,
        P_INST_CODE,
        TRIM(TO_CHAR(P_ACTUAL_AMT, '999999999999.99')),
        V_CURRCODE,
        NULL,
        NULL,
        P_TERMINAL_ID,
  ...
  WHEN OTHERS THEN
    ...
    EXCEPTION
     WHEN OTHERS THEN
       V_ERRMSG   := 'Error while updating 9 CMS_TRANSLIMIT_CHECK ' ||SUBSTR(SQLERRM, 1, 200);
       V_RESP_CDE := '21';
       RAISE EXP_RVSL_REJECT_RECORD;
    END;
       END IF;
    ...
    EXCEPTION
     WHEN OTHERS THEN
       V_ERRMSG   := 'Error while updating 10 CMS_TRANSLIMIT_CHECK ' ||SUBSTR(SQLERRM, 1, 200);
       V_RESP_CDE := '21';
       RAISE EXP_RVSL_REJECT_RECORD;
    END;
       END IF;
