CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_CALL_EXCP_DETL_REP 
AS
   ...
   IS
    SELECT mcc_file_name FileName, 
           mcc_record_numb LineNumber, 
           VMSCMS.FN_MASK(mcc_pan_code,'X',7,6) CardNumber, 
           MCC_ERRMSG ErrorMessage
      FROM VMSCMS.MIGR_CSR_CALLLOG_TEMP
     WHERE mcc_proc_flag = 'F'
	 and  mcc_file_name = prm_file_name
     order by mcc_file_name,mcc_record_numb;
...
BEGIN
   v_errmsg := 'OK';
   v_sqlerrmsg := 'OK';
   l_file_name := 'CALL_EXCP_DETL_REP.csv';
