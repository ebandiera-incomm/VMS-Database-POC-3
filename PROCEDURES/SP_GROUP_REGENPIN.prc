CREATE OR REPLACE PROCEDURE VMSCMS.Sp_Group_Regenpin (
...
AS
   ...
   v_repinavepoint          NUMBER (9)                          DEFAULT 99;
...
--Sn check for pending records
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
         v_errmsg := 'No record found for card Regenpin processing';
...
------------------------------------Sn create a record in pan spprt ----------------------------------
   ...
   EXCEPTION
      ...
      THEN
         v_errmsg :=
                'Repin reason code is not present in support master';
         RAISE exp_loop_reject_record;
      WHEN OTHERS
      THEN
         v_errmsg :=
   ...
   FOR i IN c1
   LOOP
      v_repinavepoint := v_repinavepoint + 1;
      SAVEPOINT v_repinavepoint;

      BEGIN                                             -- << LOOP I BEGIN >>
         ---------------------------------Sn Find product catg for the card---------------------
         BEGIN
            SELECT cap_prod_catg, cap_acct_id, cap_pin_off, cap_pingen_date
              INTO v_prod_catg, v_acct_id, v_oldpinoff, v_pingen_date
              FROM CMS_APPL_PAN
...
--EN create decr pan
      ...
      EXCEPTION                                       -- << LOOP I EXCEPTION>>
         ...
         THEN
            ...
            --v_errmsg := 'OK';                   -- Sn to reset the error value
         WHEN OTHERS
         THEN
            ROLLBACK TO v_repinavepoint;
            v_succ_flag := 'E';
