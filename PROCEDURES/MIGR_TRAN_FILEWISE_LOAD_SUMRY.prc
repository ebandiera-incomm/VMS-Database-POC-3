CREATE OR REPLACE PROCEDURE VMSCMS.MIGR_TRAN_FILEWISE_LOAD_SUMRY 
AS
   ...
   IS
    SELECT mfd_file_name FileName, 
           mfd_file_rec_cnt TotalRecords, 
           mfd_succ_rec_cnt LoadedRecords, 
           mfd_err_rec_cnt ErrorRecords
    FROM vmscms.migr_file_detl
    WHERE mfd_migr_type = 'TXNLOG_DATA_MIGR' 
...
BEGIN
   ...
   THEN
      ...
      UTL_FILE.put (l_file, 'TOTAL RECORDS,');
      UTL_FILE.put (l_file, 'LOADED RECORDS,');
      UTL_FILE.put (l_file, 'ERROR RECORDS,');
