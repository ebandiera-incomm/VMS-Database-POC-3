...
IS
   ...
   *************************************************************************************************/
   ...
   AS
      ...
      v_crcheck               NUMBER             := 0;
      v_drcheck               NUMBER             := 0;
      v_echeck               NUMBER              := 0; --VMS-5971
      v_fcheck               NUMBER              := 0; --VMS-5971
   BEGIN
      prm_errmsg := 'OK';
            ...
            BEGIN
               ...
               LOOP
                  BEGIN
                     ...
                     v_seg_data := SUBSTR (v_filebuffer, 25);
                      IF v_rec_type IN ('C','D','E','F') THEN  --'E' & 'F' added for VMS-5971
                       ...
                       LOOP
                        ...
                        IF TRIM(substr(v_seg_data,1,240)) IS NOT NULL THEN
                            IF v_rec_type ='C' THEN
                                v_crcheck := v_crcheck + 1;
                            ELSIF v_rec_type ='D' THEN
                                v_drcheck :=v_drcheck + 1;
                            --SN: VMS-5971
                            ELSIF v_rec_type ='E' THEN
                                v_echeck :=v_echeck + 1;
                            ELSIF v_rec_type ='F' THEN
                                v_fcheck :=v_fcheck + 1;
                            END IF;
                            --EN: VMS-5971
                        END IF;
                    ...
                    v_errmsg := 'OK';
                     BEGIN
                        IF i = 1 THEN
                           ...
                           IF v_rec_type IN ('C','D','E','F') THEN --VMS-5971
                       ...
                       IF trim(substr(v_seg_data,1,240)) IS NOT NULL THEN
                        ...
                        BEGIN
                           v_tabvar := c_ach_type ();
                           tokenise_ach (v_rec_data||v_seg_data, v_tabvar, v_errmsg);
               ...
               BEGIN
                  UPDATE cms_achfile_dtls
                     SET cad_upd_stat = 'Y',
                         cad_tot_rows = v_crcheck + v_drcheck + v_echeck + v_fcheck, --VMS-5971
                         cad_succ_rows = v_succ_row,
                         cad_err_rows = (v_crcheck + v_drcheck + v_echeck + v_fcheck) - v_succ_row, --VMS-5971
                         cad_tot_amt = v_total_damt, -- + v_total_camt, --VMS-5971
               ...
               send_mail (x.cad_file_name, v_file_errmsg);
   ...
   AS
      ...
      v_orgl_traceno    transactionlog.tracenumber%TYPE; --VMS-5971
      v_dup_recno       NUMBER;
      v_files           c_ach_type;
   ...
   BEGIN
      ...
      BEGIN
         achcanda_data_load (prm_directory,
                                prm_dest_directory,
                                prm_rej_directory,
                                prm_autoschedule,
                                v_files,
         ...
         LOOP
            ...
            LOOP
               ...
               BEGIN
                  ...
                  v_source_sname := i.achdata (11);
                  v_source_fname := i.achdata (13);
                  v_cust_name := i.achdata (12); --Added for FSS-4570
                  v_orgl_traceno := i.achdata (19); --VMS-5971

                  IF v_trandate < SYSDATE - 100 THEN
                     v_errmsg :='Txn date exceed 100 days from file creation date.';
                     v_resp_code := '902'; --'21'; --R20-902
                     RAISE excp_record_err;
                  END IF;
                 ...
                 END IF;
                  ...
                  END IF;
                     ...
                     achcanda_txn_process (prm_instcode,
                                           v_proxy_no,
                                           v_tracenumber,
               ...
               UPDATE cms_achcanda_temp
                ...
                IF v_proc_stat = 'S' AND i.cat_txn_type = 'E' THEN
                   UPDATE cms_achcanda_temp
                      SET cat_rvsl_flag = 1
                    WHERE cat_trace_no = v_orgl_traceno
                      AND cat_txn_type = 'C'
                      AND cat_proc_stat = 'S';
                END IF;
            ...
            BEGIN
               batch_load_rep (v_files (x), prm_rep_directory, v_errmsg);
               ...
               batch_excp_rep (v_files (x), prm_rep_directory, v_errmsg);
   ...
   AS
      ...
      v_del_channel            cms_transaction_mast.ctm_delivery_channel%TYPE:= '15';
...
v_Retdate  date; --Added for VMS-5739/FSP-991
   ...
   BEGIN
      ...
      --Sn Generate RRN
      BEGIN
         SELECT TO_CHAR (SYSTIMESTAMP, 'yymmddHH24MISS') || LPAD (seq_canadaach_rrn.NEXTVAL, 3, 0)
           INTO v_rrn
           FROM DUAL;
      EXCEPTION
         WHEN OTHERS THEN
      ...
      IF p_txntype IN ('C', 'E') THEN
        v_txn_code:='1';
      ELSIF p_txntype IN ('D', 'F') THEN
        v_txn_code:='2';
      END IF;
        ...
        BEGIN
          SELECT NVL(cat_rvsl_flag,0),
            ...
            INTO v_rvsl_flag,
                 ...
                 v_post_dt
            FROM cms_achcanda_temp
           WHERE cat_trace_no = p_orgl_traceno
             AND cat_txn_type = 'C'
             AND cat_proc_stat = 'S';
...
IF (v_Retdate>v_Retperiod)
    THEN
      ...
      WHEN exp_main_reject_record THEN
         ...
         IF v_dr_cr_flag IS NULL THEN
            ...
            IF p_txntype IN ('E', 'F') THEN
                v_dr_cr_flag := CASE v_dr_cr_flag WHEN 'DR' THEN 'CR' ELSE 'DR' END;
                v_trans_desc := v_trans_desc||' - Reversal';
            END IF;
         END IF;
   ...
   PROCEDURE send_mail (prm_filename IN VARCHAR2, prm_errmsg IN OUT VARCHAR2)
   AS
   ...
   PROCEDURE batch_excp_rep (prm_filename    IN     VARCHAR2,
                              prm_directory   IN     VARCHAR2,
                              prm_errmsg         OUT VARCHAR2)
    AS
        v_str       VARCHAR2 (4000);
        ...
        CURSOR c_rep IS
            SELECT    cat_proxy_no
                   ...
                   || cat_txn_type
                   || ','
                   || cat_txn_amt*100 --TRIM (TO_CHAR (cat_txn_amt, '9999999990.99'))
                   || ','
                   || cat_trace_no
                   || ','
                   || cat_err_code    AS DATA
              FROM cms_achfile_txn_errlog
             WHERE cat_file_name = prm_filename;
    BEGIN
        prm_errmsg := 'OK';
        ...
        BEGIN
            SELECT LPAD (COUNT (*)+1, 4, '0')seq
              INTO v_seq
              FROM cms_achfile_dtls
             WHERE SUBSTR (cad_file_name,(INSTR (cad_file_name,'-',1,2)+ 1),((INSTR (cad_file_name,'-',1,3)- 1) - INSTR (cad_file_name,'-',1,2))) = v_partnr
               AND cad_upd_stat = 'Y'
